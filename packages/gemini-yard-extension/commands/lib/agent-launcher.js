...
const AGENT_INFO = {
  'yard-master': { icon: '🧠', role: 'Master Orchestrator' },
...
function listAvailableAgents(projectRoot = process.cwd()) {
  const sourceDir = path.join(projectRoot, '.yard-core', 'development', 'agents');
...
function renderGreeting(projectRoot, agentId) {
  const scriptPath = path.join(projectRoot, '.yard-core', 'development', 'scripts', 'generate-greeting.js');
...
function runAgentLauncher(agentId, projectRoot = process.cwd()) {
  if (!agentId) {
    console.log('Uso: /yard-agent <agent-id>');
    return 1;
  }
