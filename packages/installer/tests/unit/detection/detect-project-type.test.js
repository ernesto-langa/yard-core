...
describe('detectProjectType', () => {
  ...
  describe('AC #1: GREENFIELD detection', () => {
    test('detects GREENFIELD when directory is empty', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .yard-core, package.json, or .git
      });
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('.yard-core'));
  ...
  describe('AC #2: BROWNFIELD detection - package.json', () => {
    test('detects BROWNFIELD when package.json exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .yard-core or .git
      });
    ...
    test('detects BROWNFIELD when only package.json exists (no git)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.yard-core')) return false;
        return false;
      });
  ...
  describe('AC #3: BROWNFIELD detection - .git', () => {
    test('detects BROWNFIELD when .git exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .yard-core or package.json
      });
  ...
  describe('AC #4: EXISTING_YARD detection', () => {
    test('detects EXISTING_YARD when .yard-core exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.yard-core')) return true; // .yard-core exists
        return true; // Other files may exist too
      });
      fs.readdirSync.mockReturnValue(['.yard-core', 'package.json', '.git', 'src']);
  ...
  describe('Edge Case: UNKNOWN detection', () => {
    test('returns UNKNOWN when directory has files but no recognized markers', () => {
      // Setup: Files exist but no package.json, .git, or .yard-core
      fs.existsSync.mockImplementation((checkPath) => {
  ...
  describe('Error Handling', () => {
    ...
    test('throws error when directory cannot be accessed (EACCES)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.yard-core')) {
  ...
  describe('Detection Priority Order', () => {
    test('priority: EXISTING_YARD > GREENFIELD', () => {
      ...
      fs.readdirSync.mockReturnValue(['.yard-core']); // Only .yard-core
