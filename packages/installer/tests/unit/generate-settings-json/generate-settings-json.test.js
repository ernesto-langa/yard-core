...
const {
  ...
  generatePermissions,
  writeSettingsJson,
} = require('../../../../../.yard-core/infrastructure/scripts/generate-settings-json');
...
function createTempProject(boundary, existingSettings) {
  ...
  fs.writeFileSync(path.join(tmpDir, '.yard-core', 'core-config.yaml'), yamlContent, 'utf8');

  // Create directory structure for expansion tests
  if (boundary.protected.includes('.yard-core/core/**')) {
    const coreDir = path.join(tmpDir, '.yard-core', 'core');
...
describe('generate-settings-json', () => {
  ...
  describe('generatePermissions — frameworkProtection: true', () => {
    test('generates deny rules covering all protected paths', () => {
      ...
      fs.mkdirSync(path.join(tmpDir, '.yard-core', 'infrastructure'), { recursive: true });
      ...
      try {
        ...
        expect(permissions.deny).toContain('Edit(.yard-core/core/events/**)');
        expect(permissions.deny).toContain('Write(.yard-core/core/events/**)');
        expect(permissions.deny).toContain('Edit(.yard-core/core/utils/**)');
        expect(permissions.deny).toContain('Write(.yard-core/core/utils/**)');
        expect(permissions.deny).toContain('Edit(.yard-core/core/index.js)');
        expect(permissions.deny).toContain('Write(.yard-core/core/index.js)');
        ...
        expect(permissions.allow).toContain('Write(.yard-core/data/**)');
        expect(permissions.allow).toContain('Read(.yard-core/**)');
      } finally {
        cleanupTempProject(tmpDir);
      }
    });
    ...
    test('all 9 protected paths from core-config produce deny rules', () => {
      ...
      const protectedRoots = [
        ...
        '.yard-core/development/checklists/',
  ...
  describe('generatePermissions — frameworkProtection: false', () => {
    test('produces no boundary deny rules', () => {
      const boundary = {
        ...
        protected: ['.yard-core/core/**', '.yard-core/infrastructure/**'],
        exceptions: ['.yard-core/data/**'],
      };
