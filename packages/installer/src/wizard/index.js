...
} = require('../../../../.yard-core/infrastructure/scripts/llm-routing/install-llm-routing');
...
async function getExistingUserProfile(targetDir = process.cwd()) {
  const coreConfigPath = path.join(targetDir, '.yard-core', 'core-config.yaml');
...
async function runWizard(options = {}) {
  try {
    ...
    if (answers.selectedTechPreset && answers.selectedTechPreset !== 'none') {
      ...
      try {
        ...
        const possiblePresetDirs = [
          path.join(__dirname, '..', '..', '.yard-core', 'data', 'tech-presets'),
          path.join(process.cwd(), '.yard-core', 'data', 'tech-presets'),
        ];
        ...
        if (sourcePresetDir) {
          ...
          if (fse.existsSync(presetFile)) {
            // Copy preset to project's .yard-core/data/tech-presets/
            const targetPresetDir = path.join(process.cwd(), '.yard-core', 'data', 'tech-presets');
            await fse.ensureDir(targetPresetDir);

            // BUG-5 fix (INS-1): Guard against source === dest (e.g., running inside yard-core repo)
            ...
            console.log(
              `   📁 Location: .yard-core/data/tech-presets/${answers.selectedTechPreset}.md`,
            );
    ...
    // Story INS-4.3: Wire settings.json boundary generator after .yard-core/ copy
    console.log('\n🔒 Generating boundary rules...');
    try {
      const settingsGenerator = require('../../../../.yard-core/infrastructure/scripts/generate-settings-json');
    ...
    // Bootstrap runs AFTER .yard-core deps are installed (yard-core-installer.js:324-345)
    // NODE_PATH ensures spawned scripts can resolve packages from .yard-core/node_modules/
    ...
    try {
      ...
      if (fse.existsSync(registryScript)) {
        ...
        // INS-4.12 AC2: Set NODE_PATH so spawned scripts resolve deps from .yard-core/node_modules/
          ...
          const registryPath = path.join(process.cwd(), '.yard-core', 'data', 'entity-registry.yaml');
          let entityCount = 0;
    ...
    try {
      ...
      if (envResult.envCreated && envResult.coreConfigCreated) {
        ...
        console.log('  - .yard-core/core-config.yaml created');
    ...
    //       console.log('\n💡 Check .yard/install-errors.log for details');
    //     }
    //
    ...
    try {
      const validation = await validateInstallation(
        {
          files: {
            ...
            env: '.env',
            coreConfig: '.yard-core/core-config.yaml',
            mcpConfig: '.mcp.json',
          },
          configs: {
      ...
      console.log('Installation may be incomplete. Check logs in .yard/ directory.');
    }
