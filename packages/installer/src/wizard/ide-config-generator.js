...
/**
 * Copy agent files from .yard-core/development/agents to IDE-specific agent folder
...
async function copyAgentFiles(projectRoot, agentFolder, ideConfig = null) {
  ...
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.yard-core', 'development', 'agents');
  ...
  for (const file of agentFiles) {
    ...
    if (stat.isFile()) {
      if (isAntiGravity) {
        ...
        try {
          const agentParser = require('../../../../.yard-core/infrastructure/scripts/ide-sync/agent-parser');
          const copilotTransformer = require('../../../../.yard-core/infrastructure/scripts/ide-sync/transformers/github-copilot');
...
async function createAntiGravityConfigJson(projectRoot, ideConfig) {
  ...
  const config = {
    ...
    paths: {
      ...
      tasks: '.yard-core/tasks',
      workflows: '.yard-core/workflows',
    },
  };
  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // Load template from .yard-core/product/templates/
        const templatePath = path.join(__dirname, '..', '..', '..', '..', '.yard-core', 'product', 'templates', ide.template);
...
/**
 * Copy .yard-core/hooks/gemini folder into .gemini/hooks during installation
 * @param {string} projectRoot - Project root directory
 * @returns {Promise<string[]>} List of copied files
 */
async function copyGeminiHooksFolder(projectRoot) {
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.yard-core', 'hooks', 'gemini');
...
async function linkGeminiExtension(projectRoot) {
  const extensionDir = path.join(projectRoot, 'packages', 'gemini-yard-extension');
