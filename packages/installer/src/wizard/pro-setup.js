 ...
 *
 * Used when @ernesto-langa/yard-pro is not yet installed (first install scenario).
...
class InlineLicenseClient {
   ...
   * @param {string} version - yard-core version
 ...
 * 2. @ernesto-langa/yard-pro package (brownfield: node_modules/@ernesto-langa/yard-pro/license/{name})
 * 3. Absolute path via yard-core in node_modules (brownfield upgrade)
 * 4. Absolute path via @ernesto-langa/yard-pro in user project (npx context)
 *
 * Path 4 is critical for npx execution: when running `npx yard-core install`,
 * require() resolves from the npx temp directory, not process.cwd(). After
 * bootstrap installs @ernesto-langa/yard-pro in the user's project, only an
 * absolute path to process.cwd()/node_modules/@ernesto-langa/yard-pro/... works.
 *
...
function loadProModule(moduleName) {
  ...
  // 2. @ernesto-langa/yard-pro package (works when yard-core is a local dependency)
  try {
    return require(`@ernesto-langa/yard-pro/license/${moduleName}`);
  } catch { /* not available */ }

  // 3. yard-core in node_modules (brownfield upgrade from >= v4.2.15)
  try {
    const absPath = path.join(process.cwd(), 'node_modules', 'yard-core', 'pro', 'license', moduleName);
    return require(absPath);
  } catch { /* not available */ }

  // 4. @ernesto-langa/yard-pro in user project (npx context — require resolves from
  //    temp dir, so we need absolute path to where bootstrap installed the package)
  try {
    const absPath = path.join(process.cwd(), 'node_modules', '@ernesto-langa', 'yard-pro', 'license', moduleName);
    return require(absPath);
  } catch { /* not available */ }
 ...
 *
 * Prefers the full LicenseApiClient from @ernesto-langa/yard-pro when available.
...
function getLicenseClient() {
  ...
  // Fallback: use inline client for pre-bootstrap (no @ernesto-langa/yard-pro yet)
...
async function stepLicenseGateWithEmail() {
  ...
  if (!checkResult.isBuyer) {
    ...
    showInfo('  Issues: https://github.com/ernesto-langa/yard-core/issues');
...
async function stepInstallScaffold(targetDir, options = {}) {
  ...
  // 1. Bundled in yard-core package (pro/ submodule — npx and local dev)
  // 2. @ernesto-langa/yard-pro in node_modules (legacy brownfield)
  const bundledProDir = path.resolve(__dirname, '..', '..', '..', '..', 'pro');
  const npmProDir = path.join(targetDir, 'node_modules', '@ernesto-langa', 'yard-pro');
