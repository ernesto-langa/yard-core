 ...
 * node_modules/@ernesto-langa/yard-pro/ into the user's project after
 * license activation.
 *
...
const SCAFFOLD_ITEMS = [
  ...
  },
  {
    ...
    dest: path.join('.yard-core', 'pro-config.yaml'),
  ...
  },
  {
    ...
    dest: path.join('.yard-core', 'feature-registry.yaml'),
 ...
 * @param {string} proSourceDir - Path to pro package content (node_modules/@ernesto-langa/yard-pro)
...
async function scaffoldProContent(targetDir, proSourceDir, options = {}) {
  ...
  if (!await fs.pathExists(proSourceDir)) {
    result.errors.push(
      `Pro package not found at ${proSourceDir}. Run "npm install @ernesto-langa/yard-pro" first.`
    );
    return result;
  }
...
async function mergeProConfig(targetDir) {
  ...
  const proConfigPath = path.join(targetDir, '.yard-core', 'pro-config.yaml');
