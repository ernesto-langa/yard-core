...
class AIOXUpdater {
    ...
    try {
      ...
      if (!result.latest) {
        ...
        if (!isOnline) {
          ...
          result.error = 'Package yard-core not found on npm registry. This may be a local development installation.';
        }
        return result;
      }
  ...
  async getInstalledVersion() {
    ...
    const packageJsonPath = path.join(this.projectRoot, 'node_modules', 'yard-core', 'package.json');
    ...
    if (fs.existsSync(localPackageJsonPath)) {
      try {
        ...
        if (pkg.name === '@synkra/yard-core' || pkg.name === 'yard-core') {
  ...
  async getLatestVersion() {
    return new Promise((resolve) => {
      const request = https.get(
        'https://registry.npmjs.org/yard-core/latest',
  ...
  async applyUpdate(targetVersion, _customizedFiles = []) {
    ...
    try {
      ...
      const cmd = `npm install yard-core@${targetVersion} --save-exact`;
      ...
      // TODO: Copy new files from node_modules to .yard-core
...
function formatCheckResult(result, options = {}) {
  ...
  switch (result.status) {
    ...
    case UpdateStatus.UPDATE_AVAILABLE:
      ...
      lines.push(`  Run ${c.cyan}npx yard-core update${c.reset} to update.`);
      break;
...
function formatUpdateResult(result, options = {}) {
  ...
  if (result.success) {
    ...
    lines.push('');
    lines.push(`Run ${c.cyan}npx yard-core validate${c.reset} to verify installation.`);
  } else {
