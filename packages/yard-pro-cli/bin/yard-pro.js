 ...
 * yard-pro CLI
 *
 * Thin CLI wrapper for @ernesto-langa/yard-pro.
 * Provides a clean npx interface: npx yard-pro install
 *
 * Commands:
 *   install             Install @ernesto-langa/yard-pro in the current project
...
const PRO_PACKAGE = '@ernesto-langa/yard-pro';
...
function isProInstalled() {
  try {
    const pkgPath = path.join(process.cwd(), 'node_modules', '@ernesto-langa', 'yard-pro', 'package.json');
...
function runProWizard(key) {
  ...
  try {
    ...
    console.error('Ensure yard-core installer is available.\n');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
yard-pro v${VERSION} — Yard Pro CLI

Usage:
  npx yard-pro <command> [options]
...
Examples:
  ...
  npx yard-pro setup
  npx yard-pro wizard --key PRO-XXXX-XXXX-XXXX-XXXX
  npx yard-pro activate --key PRO-XXXX-XXXX-XXXX-XXXX
  npx yard-pro status
  npx yard-pro recover
...
function installPro() {
  ...
  console.log('  npx yard-pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('  npx yard-pro status');
  console.log('');
}
...
if (command === '--version' || command === '-v') {
  console.log(`yard-pro v${VERSION}`);
  process.exit(0);
}
