...
/**
 * Check if a config-resolver is available (yard-core installed)
 * @param {string} projectRoot - Project root
 * @returns {Object|null} Config resolver module or null
 */
function tryLoadConfigResolver(projectRoot) {
  const possiblePaths = [
    path.join(projectRoot, '.yard-core/core/config/config-resolver.js'),
    path.join(projectRoot, 'node_modules/yard-core/.yard-core/core/config/config-resolver.js'),
  ];
...
async function createUserConfigDirect(profile, logger, dryRun) {
  const userConfigDir = path.join(os.homedir(), '.yard');
...
function detectBrownfield(projectRoot) {
  ...
  const legacyConfigPath = path.join(projectRoot, '.yard-core/core-config.yaml');
  ...
  const frameworkConfigPath = path.join(projectRoot, '.yard-core/framework-config.yaml');
...
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx yard-core doctor');
    return;
  }
...
async function installAioxCore(projectRoot, logger, dryRun) {
  ...
  const spinner = ora('Installing yard-core...').start();

  try {
    await execa('npm', ['install', 'yard-core', '--save-dev'], {
      cwd: projectRoot,
      timeout: 300000, // 5 minutes
    });

    spinner.succeed('yard-core installed');
  } catch (error) {
    spinner.fail('Failed to install yard-core');
    throw error;
  }
}
  ...
  if (brownfield.isBrownfield) {
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['yard-core', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
        ...
        logger.action('Run: npx yard-core config migrate');
      }
    } else {
