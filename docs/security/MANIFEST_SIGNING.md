...
minisign -G -p yard-core.pub -s yard-core.key

# You will be prompted for a password to protect the secret key
# CHOOSE A STRONG PASSWORD!

# Output:
#   yard-core.pub  - PUBLIC key (safe to share, will be hardcoded)
#   yard-core.key  - SECRET key (NEVER share, store securely)
```
...
- Store `yard-core.key` in a secure location (password manager, HSM, etc.)
...
2. **Sign the Manifest**
   ...
   ```bash
   cd .yard-core
   minisign -Sm install-manifest.yaml -s /path/to/yard-core.key
...
3. **Verify Signature (Optional but Recommended)**
   ...
   ```bash
   minisign -Vm install-manifest.yaml -p /path/to/yard-core.pub
...
4. **Commit Both Files**
   ...
   ```bash
   git add .yard-core/install-manifest.yaml
   git add .yard-core/install-manifest.yaml.minisig
