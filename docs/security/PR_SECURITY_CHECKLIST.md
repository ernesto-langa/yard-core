...
1. Generate a key pair: `minisign -G -p yard-core.pub -s yard-core.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s yard-core.key`
...
3. **Sign manifest** before each release:
   ...
   ```bash
   minisign -Sm .yard-core/install-manifest.yaml -s yard-core.key
   ```
