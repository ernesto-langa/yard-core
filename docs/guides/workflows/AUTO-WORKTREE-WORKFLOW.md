...
flowchart TB
    ...
    PF3 -->|Existe| PF4
    PF3 -->|Nao existe| ERR3[Erro: YARD incompleto]
...
graph TB
    ...
    subgraph FS["File System"]
        WT[.yard/worktrees/{storyId}/<br/>Worktree Directory]
        LOG[.yard/logs/merges/<br/>Merge Audit Logs]
    end
...
**Estrutura Criada:**
```
.yard/worktrees/{storyId}/     # Diretorio de trabalho
...
```bash
git worktree add .yard/worktrees/{storyId} -b auto-claude/{storyId}
```
...
| Propriedade | Valor |
|-------------|-------|
| **Localizacao** | `.yard-core/development/tasks/create-worktree.md` |
| **Agente** | @devops (Gage) |
| **Versao** | 1.0 |
| **Story** | 1.3 |
...
| **Yard Core** | Instalado | Verificar `.yard-core/` |
...
interface WorktreeInfo {
  ...
  path: string;              // '/abs/path/.yard/worktrees/STORY-42'
...
ls .yard-core/infrastructure/scripts/worktree-manager.js
```
...
3. Aumentar limite (se necessario) em `.yard/config.yaml`
...
3. Remocao manual:
   ```bash
   git worktree remove .yard/worktrees/{storyId} --force
...
| Arquivo | Caminho |
|---------|---------|
| **Workflow Definition** | `.yard-core/development/workflows/auto-worktree.yaml` |
| **Task Create** | `.yard-core/development/tasks/create-worktree.md` |
| **WorktreeManager** | `.yard-core/infrastructure/scripts/worktree-manager.js` |
...
*Documentacao gerada automaticamente pelo YARD-FULLSTACK*
