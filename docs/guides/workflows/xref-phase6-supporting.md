...
| `registry/workflow-registry.js` | Central registry of all available workflows | `index.js`, `suggestion-engine.js` | Loads from `.yard-core/development/workflows/` | No |
...
**Location:** `.yard-core/core/quality-gates/` (10 files) and `.yard-core/quality/` (4 files)
**Total Files:** 14
...
**Location:** `.yard-core/agent-teams/`
**Total Files:** 0 (directory does not exist or is empty)
**Purpose:** Was intended to hold agent team configurations.

### Status: DOES NOT EXIST

The `agent-teams/` directory was not found in the `.yard-core/` directory. However, references to "agent-teams" exist in:
...
The `tools/` directory exists (referenced in `core-config.yaml` as `toolsLocation: .yard-core/tools`) but contains no files. Tool configurations may have been:
...
- `core-config.yaml`: `toolsLocation: .yard-core/tools`
- `framework-config.yaml`: `tools_dir: ".yard-core/tools"`
...
**Location:** `.yard-core/core/elicitation/` (5 files) and `.yard-core/elicitation/` (3 files)
**Total Files:** 8
...
- **Core config:** `core-config.yaml` has `elicitationLocation: .yard-core/elicitation`
- **Framework config:** `framework-config.yaml` has `elicitation_dir: ".yard-core/elicitation"`
...
**Location:** `.yard-core/docs/standards/`
**Total Files:** 16
...
**Location:** `.yard-core/processes/`
**Total Files:** 0 (directory does not exist)
**Purpose:** Was intended to hold process definitions.

### Status: DOES NOT EXIST

No `processes/` directory was found under `.yard-core/`. This may be:
...
No references to `.yard-core/processes/` were found in the codebase.

---

## 12. Root Config System

**Location:** `.yard-core/` (root-level files)
**Total Files:** 8
...
| `user-guide.md` | User guide for YARD-Framework | Users, onboarding | Referenced by workflows, brownfield guide | No |
...
| 13 | Root Config | `.yard-core/` root | 8 | Active | In migration (monolithic to layered) |
