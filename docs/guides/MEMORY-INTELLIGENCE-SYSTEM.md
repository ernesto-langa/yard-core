...
```mermaid
flowchart LR
    subgraph CORE["yard-core (Open Source, MIT)"]
        direction TB
...
O yard-core ja possui o modulo `bin/utils/pro-detector.js` (Story PRO-5):
...
#### yard-core (open source) — Extension Points Only

| Arquivo | Tipo | Descricao |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | Existente | `isProAvailable()`, `loadProModule()` |
| `.yard-core/development/scripts/unified-activation-pipeline.js` | Modificacao | Adiciona `if (isProAvailable())` no Tier 2 para memory loading |
| `.yard-core/development/scripts/greeting-builder.js` | Modificacao | Adiciona memory stats no greeting se pro disponivel |
| `.yard-core/core/memory/gotchas-memory.js` | Mantido | Path fix (MIS-2), funciona standalone |
...
Os hooks do Claude Code sao configurados em `.claude/settings.json` (yard-core). O script apontado reside em `pro/`:
...
```javascript
// .yard-core/hooks/pro-hook-runner.js (no core, ~20 linhas)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["yard-core (Open Source)"]
        direction TB
...
### Fluxo Sem Pro (yard-core standalone)
...
```json
// .claude/settings.json (yard-core — committed)
{
  "hooks": {
    "PreCompact": [
      {
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["yard-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to<br/>.yard/memories/")]
    end
...
```
.yard/memories/                          # Criado e gerenciado por pro/memory/
...
Cada agente acessa:
1. Suas proprias memorias privadas (`.yard/memories/{agent}/`)
2. Memorias compartilhadas (`.yard/memories/shared/`)
...
### Especificacao (arquivos em .yard/memories/)
...
→ PROPOSTA: Adicionar a rules/: "Warning: .yard/ permissions"
...
**Sem pro:** Apenas `[CORE]` e `[NATIVO]` existem. `[PRO]` (.yard/memories/) nunca e criado.

---

## Mapa de Arquivos do Sistema

### yard-core (Open Source) — Extension Points

| Arquivo | Tipo | Story | Descricao |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | Existente (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.yard-core/hooks/pro-hook-runner.js` | **Novo** | MIS-3 | Wrapper: verifica pro, delega hook (~20 linhas) |
| `.yard-core/development/scripts/unified-activation-pipeline.js` | **Modificacao** | MIS-6 | Adiciona `if (isProAvailable())` no Tier 2 |
| `.yard-core/development/scripts/greeting-builder.js` | **Modificacao** | MIS-6 | Memory stats no greeting se pro disponivel |
| `.yard-core/core/memory/gotchas-memory.js` | **Path fix** | MIS-2 | Corrige `.yard/error-tracking.json` |
...
### Arquivos Removidos (MIS-2, yard-core)

| Arquivo | Linhas | Razao |
|---------|--------|-------|
| `.yard-core/core/memory/timeline-manager.js` | 746 | 0 consumers (orphan) |
| `.yard-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 consumers (orphan) |
| `.yard-core/core/memory/context-snapshot.js` | 648 | 0 consumers (orphan) |
| `.yard-core/elicitation/session-manager.js` | — | Dead code |
| `.yard/compound-analysis/` | ~14K | 0 readers (orphan data) |
