...
The Project Status feature automatically displays your current work context when any YARD agent activates. This includes:
...
3. Create `.yard/project-status.yaml` cache file
...
1. Edit `.yard-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
projectStatus:
  ...
  statusFile: .yard/project-status.yaml  # Cache file location
...
1. **Checks cache** - Looks for `.yard/project-status.yaml`
...
- **Cache Location:** `.yard/project-status.yaml`
...
All 11 YARD agents display project status:
...
3. Does `.yard-core/infrastructure/scripts/project-status-loader.js` exist?
...
A: Yes, all agents use the same cache file (`.yard/project-status.yaml`).
