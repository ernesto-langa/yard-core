...
- **Cada camada persiste de forma independente** — `.claude/` vs `.yard/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>yard-core/.claude/CLAUDE.md"]
    ...
    subgraph AIOX_FRAMEWORK["Camada 2: AIOX Framework"]
        ...
        FILE_EVO["File Evolution Tracker<br/>.yard-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.yard-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.yard-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.yard-core/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.yard-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.yard-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.yard-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_AIOX["Storage AIOX (.yard/)"]
        ...
        S_TIMELINE[".yard/timeline/"]
        S_FILE_EVO[".yard/file-evolution/"]
        S_SESSION[".yard/session-state.json"]
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.yard-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.yard-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.yard-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.yard-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.yard-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. yard-core/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. yard-core/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.yard/gotchas.json<br/>.yard/gotchas.md<br/>.yard/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.yard/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.yard/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.yard/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .yard/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.yard/session-state.json<br/>TTL: 1 hora")]
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".yard/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".yard/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".yard/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".yard/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".yard/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".yard/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .yard/session-state.json
    ...
    CL->>FS: Escreve .yard/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .yard/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .yard/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .yard/gotchas.md"]
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.yard/file-evolution/"]
        SRC2["ContextSnapshot<br/>.yard/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".yard/timeline/<br/>unified-timeline.json")]
    end
...
|---------|--------|------------|--------|
| `.yard-core/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.yard-core/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.yard-core/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.yard-core/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.yard-core/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.yard-core/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.yard-core/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.yard-core/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.yard-core/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.yard-core/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.yard-core/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |
...
| `.yard-core/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.yard-core/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.yard-core/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.yard-core/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `yard-core/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `yard-core/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
| Gotchas Memory Script | `.yard-core/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.yard-core/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.yard-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.yard-core/core/memory/timeline-manager.js` |
| Session Context Loader | `.yard-core/core/session/context-loader.js` |
| Context Detector | `.yard-core/core/session/context-detector.js` |
| Session State | `.yard-core/core/orchestration/session-state.js` |
| Unified Hook Interface | `.yard-core/hooks/unified/hook-interface.js` |
| Hook Registry | `.yard-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.yard-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.yard-core/hooks/gemini/session-end.js` |
| Core Config | `.yard-core/core-config.yaml` |
