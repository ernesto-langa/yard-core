...
```yaml
# .yard-core/development/agents/dev.md
agent:
  id: dev
  config:
...
```javascript
isProAvailable() === true
// but .yard/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .yard/session-digests/ contains memory digests
```
...
const fs = require('fs');
const digests = fs.readdirSync('.yard/session-digests');
...
**Solutions:**
1. Reduce digest count (archive old digests to `.yard/session-digests/archive/`)
