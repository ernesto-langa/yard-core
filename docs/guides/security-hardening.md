...
chmod 600 .yard/config.yaml   # Main config
chmod 600 .yard/users.json    # User database
chmod 600 .yard/sessions.json # Active sessions
...
```yaml
# .yard/config.yaml - Allowed directories configuration
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
```yaml
# .yard/config.yaml - Audit configuration
audit:
  enabled: true
...
2. Report security concerns via [GitHub Security Advisories](https://github.com/ernesto-langa/yard-core/security/advisories)
