...
Source: `.yard-core/data/agent-config-requirements.yaml`
...
| File | Purpose | Lines |
|------|---------|-------|
| `.yard-core/development/scripts/greeting-builder.js` | Main activation pipeline | 949 |
| `.yard-core/development/scripts/agent-config-loader.js` | Config loading per agent | ~400 |
| `.yard-core/development/scripts/generate-greeting.js` | CLI wrapper (3 agents) | 173 |
| `.yard-core/core/session/context-detector.js` | Session type detection | ~100 |
| `.yard-core/infrastructure/scripts/project-status-loader.js` | Git/project status | ~524 |
| `.yard-core/infrastructure/scripts/git-config-detector.js` | Git config detection | ~294 |
| `.yard-core/development/scripts/greeting-preference-manager.js` | Greeting level preference | ~146 |
| `.yard-core/development/scripts/workflow-navigator.js` | Workflow suggestions | ~200 |
| `.yard-core/core/permissions/index.js` | Permission mode badge | ~100 |
| `.yard-core/data/agent-config-requirements.yaml` | Per-agent config requirements | 369 |
...
**Resolution:** The `AgentConfigLoader` resolves dependencies by searching multiple paths: `development/`, `product/`, and root `.yard-core/` directories. The path mismatch does not cause runtime failures but is documented for accuracy.
