...
> Source: `.yard-core/development/scripts/unified-activation-pipeline.js` (Story ACT-6)
> Previous source: `.yard-core/development/scripts/greeting-builder.js` (949 lines)
...
Current source:
- `.yard-core/development/scripts/activation-runtime.js`
- `.yard-core/development/scripts/unified-activation-pipeline.js`
...
```
.yard-core/development/agents/{agent-id}.md
```
...
sequenceDiagram
    ...
    alt Cache hit
        ...
        ACL->>FS: readFile(.yard-core/development/agents/{id}.md)
...
**Cache file:** `.yard/project-status.yaml`
...
Reads from `.yard-core/core-config.yaml` path `agentIdentity.greeting.preference`.
...
await mode.load();  // Reads .yard/config.yaml -> permissions.mode
...
1. Reads current mode from `.yard/config.yaml`
...
```javascript
const { enforcePermission } = require('./.yard-core/core/permissions');
...
The `environment-bootstrap` task initializes `.yard/config.yaml` with `permissions.mode: ask` as the default. If the config file is missing or the field is absent, the system defaults to `ask` mode.
...
Each agent has specific config requirements defined in `.yard-core/data/agent-config-requirements.yaml`:
...
| File | Loader | Purpose |
|------|--------|---------|
| `.yard-core/development/agents/{agent-id}.md` | AgentConfigLoader | Agent definition |
| `.yard-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| `.yard-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Per-agent config needs |
| `.yard-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |

### Loaded conditionally

| File | Condition | Loader |
|------|-----------|--------|
| `.yard/session-state.json` | Path B (CLI wrapper) or file-based session detection | ContextDetector / SessionContextLoader |
| `.yard/project-status.yaml` | Cache check (60s TTL) | ProjectStatusLoader |
...
graph TD
    ...
    SCL --> SSF[.yard/session-state.json]
    PSL --> GIT[git CLI commands]
    PSL --> STORIES[docs/stories/**/*.md]
    PSL --> WTM[WorktreeManager]
    PSL --> PSC[.yard/project-status.yaml]
    GCD_R --> GIT
    WN --> WP[.yard-core/data/workflow-patterns.yaml]
    GPM --> CC
```
...
|---|------|----------|----------------|---------------------|
| 1 | `.yard-core/core-config.yaml` | Config | `user_profile: bob` | `user_profile: advanced` |
| 2 | `.yard-core/development/scripts/greeting-builder.js` | Greeting | Redirects non-PM agents to @pm; hides role/status sections; returns empty commands for non-PM | Full contextual greeting with all sections and commands |
| 3 | `.yard-core/development/scripts/generate-greeting.js` | Greeting | Uses GreetingBuilder, same bob restrictions | Uses GreetingBuilder, full features |
| 4 | `.yard-core/development/scripts/greeting-preference-manager.js` | Greeting | Forces preference to `minimal` or `named`; overrides `auto`/`archetypal` | All 4 preferences available (`auto`, `minimal`, `named`, `archetypal`) |
| 5 | `.yard-core/infrastructure/scripts/validate-user-profile.js` | Validation | Validates `bob` as legal value; normalizes case | Validates `advanced` as legal value; normalizes case |
| 6 | `.yard-core/core/config/config-resolver.js` | Config | `toggleUserProfile()` switches bob<->advanced; L5 user layer has priority | Same toggle; resolveConfig merges layers |
| 7 | `.yard-core/core/config/migrate-config.js` | Config | Categorizes `user_profile` as USER_FIELD during migration | Same categorization |
| 8 | `.yard-core/core/config/schemas/user-config.schema.json` | Schema | `enum: ["bob", "advanced"]` validation | Same validation |
| 9 | `.yard-core/core/config/templates/user-config.yaml` | Template | Default template value: `bob` | N/A (template default is bob) |
| 10 | `.yard-core/development/agents/pm.md` | Agent | PM becomes sole orchestrator; bob mode session detection; orchestrates other agents internally | PM operates as normal PM with standard workflow |
