...
flowchart TB
    subgraph CLAUDE_NATIVE["第一层: Claude Code 原生"]
        ...
        CLAUDE_MD_P["CLAUDE.md 项目<br/>yard-core/.claude/CLAUDE.md"]
    ...
    subgraph YARD_FRAMEWORK["第二层: YARD 框架"]
        ...
        FILE_EVO["文件演化追踪器<br/>.yard-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.yard-core/core/memory/timeline-manager.js"]
        CTX_LOADER["会话上下文加载器<br/>.yard-core/core/session/context-loader.js"]
        CTX_DETECT["上下文检测器<br/>.yard-core/core/session/context-detector.js"]
        SESS_STATE["会话状态<br/>.yard-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.yard-core/hooks/gemini/"]
        HOOKS_UNI["统一 Hooks<br/>.yard-core/hooks/unified/"]
    end
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["记忆层 (.yard-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["会话层 (.yard-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["编排 (.yard-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["激活管道 (.yard-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks 系统 (.yard-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["自动加载 — 固定顺序"]
        ...
        LOAD_PROJECT["3. yard-core/.claude/CLAUDE.md<br/>项目级指令"]
        LOAD_RULES["4. yard-core/.claude/rules/*.md<br/>5 个详细规则文件"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("持久化到:<br/>.yard/gotchas.json<br/>.yard/gotchas.md<br/>.yard/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("持久化到:<br/>.yard/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("持久化到:<br/>.yard/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — 统一门面"]
        ...
        T4[("持久化到:<br/>.yard/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD2{"有对话<br/>历史?"}
        CD3["分析最近命令"]
        CD4["读取 .yard/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("持久化到:<br/>.yard/session-state.json<br/>TTL: 1 小时")]
...
flowchart TD
    ...
    subgraph SAVES["保存的内容"]
        S1[".yard/session-state.json<br/>当前代理, 命令, 工作流"]
        S2[".yard/error-tracking.json<br/>错误发生"]
        S3[".yard/gotchas.json<br/>新自动捕获的 gotcha"]
        S4[".yard/gotchas.json<br/>添加的手动 gotcha"]
        S5["docs/stories/.session-state.yaml<br/>更新的阶段"]
        S6["docs/stories/.session-state.yaml<br/>Story 标记完成"]
        S7[".yard/snapshots/{id}.json<br/>捕获的上下文"]
        S8[".yard/file-evolution/<br/>记录的演化"]
    end
...
sequenceDiagram
    ...
    CL->>FS: 读取 .yard/session-state.json
    FS-->>CL: 当前状态 (或空)
    ...
    CL->>FS: 写入 .yard/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["自动捕获流程"]
        ...
        TRACK --> READ_ET["读取 .yard/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["保存 .yard/gotchas.json"]
        SAVE_JSON --> SAVE_MD["生成 .yard/gotchas.md"]
...
flowchart TD
    subgraph CAPTURE["捕获 — context-snapshot.js"]
        ...
        C4 --> C5["保存 .yard/snapshots/{id}.json"]
        C5 --> C6{"autoCleanup?"}
        C6 -->|"是"| C7["移除快照:<br/>- > 50 个存在<br/>- > 7 天"]
    end

    subgraph RESTORE["恢复 — 恢复"]
        R1(["snapshot.restore(id)"]) --> R2["读取 .yard/snapshots/{id}.json"]
        R2 --> R3["返回完整上下文"]
        R3 --> R4["代理可以从<br/>上次位置恢复"]
    end

    subgraph LIST["列出 — 查询"]
        L1(["snapshot.list()"]) --> L2["列出 .yard/snapshots/ 中所有"]
        L2 --> L3["按时间戳排序"]
        L3 --> L4["返回元数据数组"]
    end
...
| 文件 | 模块 | Story/Epic | 功能 |
|---------|--------|------------|--------|
| `.yard-core/core/memory/gotchas-memory.js` | 记忆 | Epic 9, Story 9.4 | 自动捕获重复错误, 手动 gotchas, 注入任务 |
| `.yard-core/core/memory/context-snapshot.js` | 记忆 | Story 12.6 | 捕获和恢复开发上下文 |
| `.yard-core/core/memory/file-evolution-tracker.js` | 记忆 | Gap impl | 追踪文件演化, 检测漂移 |
| `.yard-core/core/memory/timeline-manager.js` | 记忆 | Gap impl | 跨会话统一 timeline 门面 |
| `.yard-core/core/session/context-loader.js` | 会话 | Story 2.2, 6.1.2.5 | 代理间连续性, 上下文交接 |
| `.yard-core/core/session/context-detector.js` | 会话 | Story 2.2 | 混合会话类型检测 (new/existing/workflow) |
| `.yard-core/core/orchestration/session-state.js` | 编排 | Story 11.5 | Epic/story 持久状态, 崩溃恢复 |
| `.yard-core/core/orchestration/context-manager.js` | 编排 | Legacy | 阶段间工作流状态 (迁移到 session-state) |
| `.yard-core/core/elicitation/session-manager.js` | Elicitation | — | Elicitation 会话保存/加载 |

### 激活脚本 (消费记忆)

| 文件 | 功能 |
|---------|--------|
| `.yard-core/development/scripts/unified-activation-pipeline.js` | 主编排器 — 在 Tier 3 加载会话 |
| `.yard-core/development/scripts/greeting-builder.js` | 构建带会话/记忆上下文的 greeting |
...
| `.yard-core/hooks/gemini/after-tool.js` | 两者 | 工具后后处理 |
| `.yard-core/hooks/unified/hook-interface.js` | 两者 | UnifiedHook 基类 + EVENT_MAPPING |
| `.yard-core/hooks/unified/hook-registry.js` | 两者 | hooks 中央注册表 |
| `.yard-core/hooks/unified/index.js` | 两者 | 统一系统入口点 |
...
| `yard-core/.claude/CLAUDE.md` | 项目指令 (始终加载) |
| `yard-core/.claude/rules/*.md` | 5 个规则文件 (始终加载) |
...
| Gotchas 记忆脚本 | `.yard-core/core/memory/gotchas-memory.js` |
| 上下文快照脚本 | `.yard-core/core/memory/context-snapshot.js` |
| 文件演化追踪器 | `.yard-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.yard-core/core/memory/timeline-manager.js` |
| 会话上下文加载器 | `.yard-core/core/session/context-loader.js` |
| 上下文检测器 | `.yard-core/core/session/context-detector.js` |
| 会话状态 | `.yard-core/core/orchestration/session-state.js` |
| 统一 Hook 接口 | `.yard-core/hooks/unified/hook-interface.js` |
| Hook 注册表 | `.yard-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.yard-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.yard-core/hooks/gemini/session-end.js` |
| Core 配置 | `.yard-core/core-config.yaml` |
