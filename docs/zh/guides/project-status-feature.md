...
3. 创建 `.yard/project-status.yaml` 缓存文件
...
1. 编辑 `.yard-core/core-config.yaml`：
   ```yaml
   projectStatus:
     enabled: true
...
projectStatus:
  ...
  statusFile: .yard/project-status.yaml  # 缓存文件位置
...
1. **检查缓存** - 查找 `.yard/project-status.yaml`
2. **验证TTL** - 缓存 < 60秒？
...
3. `.yard-core/infrastructure/scripts/project-status-loader.js` 存在吗？
4. Agent激活输出中有错误吗？
...
**Q: 缓存在agent之间共享吗?**
A: 是的，所有agent使用相同的缓存文件 (`.yard/project-status.yaml`)。
...
- **测试:** `.yard-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
