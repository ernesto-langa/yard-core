...
> **架构:** MIS 遵循 AIOX 的 **Open Core** 模型。所有记忆智能都位于私有仓库 `aiox-pro`（子模块 `pro/`）中。`yard-core`（开源）仅提供扩展点，并且在没有智能记忆的情况下 100% 正常运行 — 与现在完全一样。
...
| **检索** | 按相关性渐进式检索 | **aiox-pro** + **yard-core** 扩展点 | MIS-4, MIS-6 |
| **演化** | 自动学习和演化规则 | **aiox-pro** | MIS-5, MIS-7 |

### 关键原则 (vs 当前状态)

| 方面 | 当前状态 | 使用 MIS (aiox-pro 激活) | 没有 MIS (仅 yard-core) |
|---------|-------------|--------------------------|--------------------------|
...
> **yard-core 在没有 MIS 的情况下 100% 正常运行** — 与现在的运行方式完全相同。当 `aiox-pro` 存在时（子模块 `pro/`），MIS 通过扩展点自动连接。无需手动配置。
...
| 方面 | 当前 yard-core | yard-core + aiox-pro (MIS) | 没有 pro 的 yard-core |
|---------|---------------|---------------------------|------------------|
...
| MIS-3 | 会话摘要 (PreCompact Hook) | **yard-core** (hook runner) + **aiox-pro** (digest) | MIS-1 | 14h |
| MIS-4 | 渐进式记忆检索 | **aiox-pro** | MIS-3 | 16h |
| MIS-5 | 自学习引擎 | **aiox-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | 管道集成与代理记忆 API | **yard-core** (扩展点) + **aiox-pro** (loader) | MIS-4 | 10h |
