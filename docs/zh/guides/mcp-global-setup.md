...
> 为YARD-Framework配置全局MCP（模型上下文协议）服务器。
...
MCP全局系统允许你配置一次MCP服务器并在所有YARD项目之间共享。这消除了在每个项目中配置相同服务器的需要。
...
- `~/.yard/` - 全局YARD目录
- `~/.yard/mcp/` - MCP配置目录
- `~/.yard/mcp/global-config.json` - 主配置文件
- `~/.yard/mcp/servers/` - 个别服务器配置
- `~/.yard/mcp/cache/` - 响应缓存
- `~/.yard/credentials/` - 安全凭据存储
...
位置: ~/.yard/mcp/global-config.json
状态:   ✓ 已配置

服务器: 0个已配置
缓存:   为空

运行 'yard mcp add <server>' 来添加服务器。
```
...
凭据存储在 `~/.yard/credentials/` 中，包含 `.gitignore` 以防止意外提交。
...
| 目录存在 | 使用 `yard mcp setup --force` 来重新创建 |
| 未找到路径 | 确保主目录存在 |
