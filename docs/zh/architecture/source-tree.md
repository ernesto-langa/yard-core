...
**迁移通知**: 此文档将在 Q2 2026 迁移到 `ernesto-langa/yard-core` 存储库（参见决策 005）
...
- [概述](#概述)
- [当前结构 (yard-core Brownfield)](#当前结构-yard-core-brownfield)
...
## 当前结构 (yard-core Brownfield)

```
yard-core/                             # 根目录 (brownfield 项目)
├── .yard-core/                        # 框架核心 (可移植)
│   ├── core/                          # 框架必要组件 (v4)
│   │   ├── config/                    # 配置系统
│   │   ├── data/                      # 核心知识库
...
### 存储库 1: ernesto-langa/yard-core (MIT)
...
```yaml
# 我在创建新代理:
位置: .yard-core/agents/{agent-name}.md
示例: .yard-core/agents/security-expert.md

# 我在创建新任务:
位置: .yard-core/tasks/{task-name}.md
示例: .yard-core/tasks/deploy-to-production.md

# 我在创建新工作流:
位置: .yard-core/workflows/{workflow-name}.yaml
示例: .yard-core/workflows/continuous-deployment.yaml

# 我在创建新模板:
位置: .yard-core/product/templates/{template-name}-tmpl.{yaml|md}
示例: .yard-core/product/templates/deployment-plan-tmpl.yaml
...
# 我在创建实用脚本:
位置: .yard-core/utils/{utility-name}.js
示例: .yard-core/utils/performance-monitor.js
