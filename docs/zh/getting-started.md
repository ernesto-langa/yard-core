...
```bash
# 新项目
npx yard-core init my-first-project
cd my-first-project
...
- Gemini CLI：`/yard-menu` 然后 `/yard-<agent>`
- Codex CLI：`/skills` 然后 `yard-<agent-id>`
...
```bash
# 1. 使用自定义模板创建新项目
npx yard-core init my-project --template enterprise
...
# 3. 在非空目录中强制安装
npx yard-core init my-project --force

# 4. 跳过依赖安装（稍后手动安装）
npx yard-core init my-project --skip-install
```
...
YARD 配置位于 `.yard-core/core/config/`。安装程序处理初始设置。要验证您的安装：
...
YARD 代理通过您的 IDE 激活。激活后，代理响应以 `*` 为前缀的命令：
...
并非所有 IDE 都同等支持 YARD 功能。请参阅 [`docs/ide-integration.md`](./ide-integration.md) 中的完整比较。
...
| Gemini CLI | 可用 | `/yard-menu` 然后 `/yard-<agent>` |
| Codex CLI | 有限 | `/skills` 然后 `yard-<agent-id>` |
...
当您首次在现有项目中激活 YARD 代理时：

1. **检测**：YARD 检测到代码但没有 YARD 文档
...
- **非破坏性**：YARD 创建文件，从不覆盖现有文件
- **回滚**：`git checkout HEAD~1 -- .` 恢复 YARD 之前的状态
- **配置保留**：您的 `.eslintrc`、`tsconfig.json` 等保持不变

### 资源

- **[棕地项目工作指南](.yard-core/working-in-the-brownfield.md)** - 完整的棕地项目文档
- **[兼容性检查清单](.yard-core/development/checklists/brownfield-compatibility-checklist.md)** - 迁移前/后检查
- **[风险报告模板](.yard-core/product/templates/brownfield-risk-report-tmpl.yaml)** - 分阶段风险评估
...
所有 YARD 开发都遵循 `docs/stories/` 中的故事。每个故事包含：
- 带复选框的验收标准
- 映射到特定 AC 的任务
- CodeRabbit 集成用于自动审查
- 质量门控分配
...
小队将 YARD 扩展到软件开发之外的任何领域。请参阅[小队指南](./guides/squads-guide.md)。
...
- **[小队指南](./guides/squads-guide.md)** - 将 YARD 扩展到任何领域
