...
### 问题 1："npx @synkra/yard-core is not recognized"
...
```
npm ERR! code E404
npm ERR! 404 Not Found - GET https://registry.npmjs.org/yard-core
```
...
npm ERR! network request to https://registry.npmjs.org/@synkra/yard-core failed
```
...
# 以详细日志重试
npm install @synkra/yard-core --verbose
```
...
```
Error: ENOTEMPTY: directory not empty, rmdir '.yard-core'
```
...
```bash
# 备份现有安装
mv .yard-core .yard-core.backup

# 使用强制标志运行安装程序
npx @synkra/yard-core install --force-upgrade

# 如果需要，从备份恢复自定义文件
cp .yard-core.backup/custom-files/* .yard-core/
```
...
4. 为特定 IDE 重新运行安装：
   ```bash
   npx @synkra/yard-core install --ide claude-code
   ```
...
```
Error: Agent 'dev' not found in .yard-core/agents/
```
...
# 如果缺失，重新安装核心
npx @synkra/yard-core install --full

# 检查 core-config.yaml 有效
cat .yard-core/core-config.yaml
```
...
```bash
# 验证 YAML 语法
npx yaml-lint .yard-core/agents/dev.md
...
# 重新安装以获取干净的代理文件
mv .yard-core/agents/dev.md .yard-core/agents/dev.md.backup
npx @synkra/yard-core install --full
```
...
# 更新现有安装
npx @synkra/yard-core update

# 显示详细日志
npx @synkra/yard-core install --verbose
```
...
```bash
# 验证 .yard-core 结构
find .yard-core -type f | wc -l
# 预期：200+ 文件

# 检查损坏的 YAML
for f in .yard-core/**/*.yaml; do npx yaml-lint "$f"; done
...
1. 运行 `npx @synkra/yard-core status` 并记录输出
2. 查看此故障排除指南
3. 搜索现有 [GitHub 问题](https://github.com/ernesto-langa/yard-core/issues)
...
- **GitHub Issues**: [yard-core/issues](https://github.com/ernesto-langa/yard-core/issues)
