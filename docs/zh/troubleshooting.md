...
```bash
# 基本诊断
npx @synkra/yard-core doctor

# 自动修复常见问题
npx @synkra/yard-core doctor --fix

# 详细输出
npx @synkra/yard-core doctor --verbose

# 检查特定组件
npx @synkra/yard-core doctor --component memory-layer
```
...
# 或直接使用 npm
npm exec @synkra/yard-core init my-project
```
...
# 在用户目录中安装
cd ~
npx @synkra/yard-core init my-project
```
...
# 跳过依赖安装
npx @synkra/yard-core init my-project --skip-install
...
# 验证配置
npx @synkra/yard-core doctor --component config
...
# 或使用更短的路径
cd C:\
npx @synkra/yard-core init myapp
```
...
```bash
# 生成完整诊断
npx @synkra/yard-core doctor --report diagnostic.json
...
1. **运行诊断：**
   ```bash
   npx @synkra/yard-core doctor --verbose > diagnostic.log
   ```
...
3. **检查现有问题：**
   ...
   - [讨论](https://github.com/yard-core/yard-core/discussions)
...
- **Stack Overflow**：使用 `yard-core` 标签提问
...
**记住**：大多数问题可以通过以下方式解决：
1. `npx @synkra/yard-core doctor --fix`
2. 清除缓存
3. 更新到最新版本
4. 检查权限
