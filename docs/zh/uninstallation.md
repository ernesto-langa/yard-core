...
```bash
# 创建完整备份
npx @synkra/yard-core backup --complete
...
```bash
# 基本卸载（保留用户数据）
npx @synkra/yard-core uninstall

# 完全卸载（删除所有内容）
npx @synkra/yard-core uninstall --complete

# 卸载并保留数据
npx @synkra/yard-core uninstall --keep-data
```
...
```bash
npx @synkra/yard-core uninstall --interactive
```
...
```bash
# 完全移除
npx @synkra/yard-core uninstall --complete --no-backup
```
...
```bash
# 仅删除代理
npx @synkra/yard-core uninstall agents

# 仅删除工作流
npx @synkra/yard-core uninstall workflows

# 删除内存层
npx @synkra/yard-core uninstall memory-layer
...
```bash
# Linux/macOS
sudo npx @synkra/yard-core uninstall --complete
...
1. **检查文档**
   ...
   - [故障排除](https://github.com/ernesto-langa/yard-core/wiki/troubleshooting)
