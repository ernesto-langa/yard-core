...
### Por que usar YARD com Gemini CLI?
...
## Ativação
Inclua "Como agente YARD dev" no seu prompt.
...
```bash
# Agente desenvolvedor
gemini "Como agente YARD dev, implemente a feature de login"

# Agente QA
gemini "Como agente YARD qa, revise este código para segurança"

# Agente arquiteto
gemini "Como agente YARD architect, projete o schema do banco de dados"
```
...
cat src/auth.ts | gemini "Como YARD qa, revise este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como YARD qa, revise este arquivo" >> reviews.md
done
```
...
1. Instale YARD para Gemini CLI:
   ```bash
...
1. Agentes YARD em `.yard-core/` são portáveis
