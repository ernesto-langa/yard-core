...
chmod 600 .yard/config.yaml   # Configuração principal
chmod 600 .yard/users.json    # Banco de dados de usuários
chmod 600 .yard/sessions.json # Sessões ativas
...
```yaml
# .yard/config.yaml - Configuração de diretórios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
```yaml
# .yard/config.yaml - Configuração de auditoria
audit:
  enabled: true
...
2. Reporte preocupações de segurança via [GitHub Security Advisories](https://github.com/ernesto-langa/yard-core/security/advisories)
