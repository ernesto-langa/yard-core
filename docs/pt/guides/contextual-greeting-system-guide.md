...
O Sistema de Greeting Contextual é uma melhoria de UX que torna os greetings dos agentes YARD inteligentes e adaptativos, mostrando informações e comandos relevantes baseados no contexto da sessão.
...
### Scripts Core
- `.yard-core/core/session/context-detector.js` - Detecção de tipo de sessão
- `.yard-core/infrastructure/scripts/git-config-detector.js` - Detecção de git config
- `.yard-core/development/scripts/greeting-builder.js` - Montagem do greeting
- `.yard-core/development/scripts/workflow-navigator.js` - Navegação de workflow
- `.yard-core/development/scripts/agent-exit-hooks.js` - Hooks de saída (para persistência)

### Arquivos de Dados
- `.yard-core/data/workflow-patterns.yaml` - Definições de workflows
...
### Configuração
- `.yard-core/core-config.yaml` - Configuração global (seções git + agentIdentity)

### Agentes (Atualizados)
- `.yard-core/agents/dev.md` - Metadata de visibilidade de comandos
- `.yard-core/agents/po.md` - Metadata de visibilidade de comandos
- `.yard-core/agents/*.md` - 9 agentes restantes (atualização pendente)
...
R: Use o script de teste: `node .yard-core/development/scripts/test-greeting-system.js`
