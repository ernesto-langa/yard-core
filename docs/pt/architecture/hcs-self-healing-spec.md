...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do YARD (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.yard/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.yard/cache/`     | Sim           |
...
- id: PC-001
  description: '.yard/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .yard/config.yaml (backup: .yard/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .yard/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx yard-core task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx yard-core task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .yard/)
...
safeToAutoFix:
  paths:
    - '.yard/**' # Arquivos de workspace do YARD
    - '.yard-core/**/*.yaml' # YAML do framework (cuidado)
