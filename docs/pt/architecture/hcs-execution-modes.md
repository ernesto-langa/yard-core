...
Este documento define os modos de execução para o Sistema de Verificação de Saúde do YARD (HCS), baseado em pesquisa de melhores práticas da indústria do Kubernetes, VS Code, Terraform, npm/yarn e padrões CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Na instalação/bootstrap**  | `npx yard-core install` | 60-120s | Esperado                       | Validação de setup      | ✅ **Obrigatório**           |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx yard-core health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path.join(process.cwd(), '.yard', reports/health-check-*.md
```
...
**Veredito:** Obrigatório. Parte do `npx yard-core install` e `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Upload Relatório
        ...
        with:
          ...
          path.join(process.cwd(), '.yard', reports/
      ...
      - name: Postar no Slack (em caso de falha)
        ...
        with:
          payload: |
            {
              "text": "⚠️ YARD Health Check Falhou",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx yard-core health-check --mode=quick
```
