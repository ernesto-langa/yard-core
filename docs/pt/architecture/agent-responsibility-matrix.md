...
Este documento define limites claros de responsabilidade para todos os agentes YARD, com foco particular em:
...
1. **Git Pre-Push Hook** (Aplicação Primária)
   ...
   - Verificações: Variável de ambiente `$YARD_ACTIVE_AGENT`
   - Ação: Bloqueia push se agent != "github-devops"

2. **Variáveis de Ambiente** (Detecção em Runtime)
   ```bash
   export YARD_ACTIVE_AGENT="github-devops"
   export YARD_GIT_PUSH_ALLOWED="true"
   ```
...
- [ ] **Criar Git Pre-Push Hook**
  - Localização: `.git/hooks/pre-push`
  - Conteúdo: Verificar `$YARD_ACTIVE_AGENT`, bloquear se != "github-devops"
...
- [ ] **Atualizar Scripts de Ativação de Agentes**
  - Adicionar configuração de variável de ambiente: `YARD_ACTIVE_AGENT={agent_id}`
  - Configurar `YARD_GIT_PUSH_ALLOWED` apropriadamente
