...
Todo o estado do ADE é persistido em `.yard/` para:
...
Arquivo de Estado: .yard/project-status.yaml
```
...
| ------------------- | ---------------------------------- | --------------------------------- |
| Padrões de Código   | Padrões reutilizáveis da base     | .yard/patterns/code-patterns.json |
| Gotchas             | Problemas conhecidos e soluções   | .yard/patterns/gotchas.json       |
| Insights de Sessão  | Descobertas durante sessões       | .yard/sessions/                   |
| Mapa de Codebase    | Análise de estrutura do projeto   | .yard/codebase-map.json           |
...
Todos os componentes do ADE atualizam `.yard/status.json`:
...
Localizada em `.yard-core/core-config.yaml`:
