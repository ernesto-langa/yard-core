...
**Propósito:** Verificar se os arquivos do framework YARD estão corretamente configurados e consistentes.

### Verificações

| ID     | Nome                           | Severidade | Nível | Modo  | Timeout | Descrição                                            |
| ------ | ------------------------------ | ---------- | ----- | ----- | ------- | ---------------------------------------------------- |
| PC-001 | Config existe                  | CRITICAL   | 1     | quick | 100ms   | `.yard/config.yaml` existe e é YAML válido           |
...
| PC-006 | Sem arquivos órfãos            | LOW        | 3     | full  | 5s      | Todos os arquivos em `.yard-core/` são referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    const configPath = path.join(context.projectRoot, '.yard', 'config.yaml');

    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.yard/config.yaml não encontrado',
  ...
  async heal(context) {
    const templatePath = '.yard-core/infrastructure/templates/core-config/config-template.yaml';
    const configPath = path.join(context.projectRoot, '.yard', 'config.yaml');
...
const requiredSections = [
  'Project Overview', // ou 'YARD-FULLSTACK Development Rules'
...
```yaml
# .yard-core/health-check/checks/ide-checks.yaml
checks:
  - id: IDE-VSCODE
    ...
    validation:
      type: json-schema
      schema: '.yard-core/schemas/vscode-settings.json'
    autoFix:
      tier: 2
      action: 'create-from-template'
      template: '.yard-core/infrastructure/templates/ide/vscode-settings.json'
...
Usuários podem definir verificações específicas do projeto em `.yard/custom-checks.yaml`:

```yaml
# .yard/custom-checks.yaml
version: 1.0
...
checks:
  ...
  - id: CUSTOM-001
    ...
    autoFix:
      action: create-from-template
      template: '.yard-core/infrastructure/templates/project-docs/readme-template.md'
  ...
  - id: CUSTOM-005
    ...
    type: json-schema
    path: 'package.json'
    schema: '.yard-core/schemas/package-json.json'
    severity: CRITICAL
    tier: 3
    mode: quick
```
