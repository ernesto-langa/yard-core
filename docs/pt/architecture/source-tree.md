...
# Estrutura da Árvore de Código YARD
...
**Aviso de Migração:** Este documento será migrado para o repositório `ernesto-langa/yard-core` no Q2 2026 (veja Decisão 005)
...
- [Estrutura Atual (yard-core Brownfield)](#estrutura-atual-yard-core-brownfield)
...
YARD utiliza uma **arquitetura de camada dupla**:

1. **Framework Core** (`.yard-core/`) - Componentes portáteis do framework
...
## Estrutura Atual (yard-core Brownfield)

```
yard-core/                             # Raiz (projeto brownfield)
├── .yard-core/                        # Core do framework (portátil)
...
│
├── bin/                               # Executáveis CLI
│   ├── yard-core.js              # Ponto de entrada principal da CLI
│   └── yard-minimal.js                # CLI mínima
│
...
**Propósito:** Componentes portáteis do framework que funcionam em qualquer projeto YARD.
...
Squads são extensões modulares que adicionam capacidades especializadas ao YARD. Diferente dos Squads descontinuados, Squads seguem uma estrutura de template padronizada.
...
│   └── cli/                           # Interface CLI
│
├── .yard-core/                        # Ativos do framework (estrutura atual)
│   ├── agents/
│   ├── tasks/
│   ├── templates/
│   └── ...
...
Localização: .yard-core/agents/{agent-name}.md
Exemplo: .yard-core/agents/security-expert.md

# Estou criando uma nova tarefa:
Localização: .yard-core/tasks/{task-name}.md
Exemplo: .yard-core/tasks/deploy-to-production.md

# Estou criando um novo workflow:
Localização: .yard-core/workflows/{workflow-name}.yaml
Exemplo: .yard-core/workflows/continuous-deployment.yaml

# Estou criando um novo template:
Localização: .yard-core/product/templates/{template-name}-tmpl.{yaml|md}
Exemplo: .yard-core/product/templates/deployment-plan-tmpl.yaml

# Estou escrevendo uma story:
Localização: docs/stories/{epic-context}/{story-file}.md
Exemplo: docs/stories/yard migration/story-6.1.2.6.md
...
Localização: .yard-core/utils/{utility-name}.js
Exemplo: .yard-core/utils/performance-monitor.js
...
| 1.1    | 2025-12-14 | Atualizado org para YardAI, substituído Squads pelo sistema Squads [Story 6.10] | Dex (dev)        |
