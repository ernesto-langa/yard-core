...
# Método YARD: Arquitectura Central
...
Todo el ecosistema del Método YARD está diseñado alrededor del directorio `yard-core` instalado, que actúa como el cerebro de la operación. El directorio `tools` proporciona los medios para procesar y empaquetar este cerebro para diferentes entornos.
...
El directorio `yard-core` contiene todas las definiciones y recursos que dan a los agentes sus capacidades.

### 3.1. Agentes (`yard-core/agents/`)

- **Propósito**: Estos son los bloques de construcción fundamentales del sistema. Cada archivo markdown (por ejemplo, `yard-master.md`, `pm.md`, `dev.md`) define la persona, capacidades y dependencias de un único agente de IA.
...
- **Ejemplo**: El agente `yard-master` lista sus dependencias, lo que indica a la herramienta de construcción qué archivos incluir en un paquete web e informa al agente de sus propias capacidades.

### 3.2. Equipos de Agentes (`yard-core/agent-teams/`)
...
### 3.3. Flujos de Trabajo (`yard-core/workflows/`)

- **Propósito**: Los flujos de trabajo son archivos YAML (por ejemplo, `greenfield-fullstack.yaml`) que definen una secuencia prescrita de pasos e interacciones de agentes para un tipo de proyecto específico. Actúan como una guía estratégica para el usuario y el agente `yard-orchestrator`.
...
- **Propósito**: Estas carpetas albergan los componentes modulares que son cargados dinámicamente por los agentes según sus dependencias.
  ...
  - **`data/`**: Contiene la base de conocimientos central (`yard-kb.md`), preferencias técnicas (`technical-preferences.md`) y otros archivos de datos clave.

#### 3.4.1. Sistema de Procesamiento de Plantillas

Un principio arquitectónico clave de YARD es que las plantillas son autónomas e interactivas - incorporan tanto la salida de documento deseada como las instrucciones LLM necesarias para trabajar con los usuarios. Esto significa que en muchos casos, no se necesita una tarea separada para la creación de documentos, ya que la plantilla misma contiene toda la lógica de procesamiento.

El framework YARD emplea un sistema sofisticado de procesamiento de plantillas orquestado por tres componentes clave:

- **`template-format.md`** (`yard-core/utils/`): Define el lenguaje de marcado fundamental usado en todas las plantillas de YARD. Esta especificación establece reglas de sintaxis para sustitución de variables (`{{placeholders}}`), directivas de procesamiento solo para IA (`[[LLM: instructions]]`) y bloques de lógica condicional. Las plantillas siguen este formato para asegurar un procesamiento consistente en todo el sistema.
...
- **`advanced-elicitation.md`** (`yard-core/tasks/`): Proporciona una capa de refinamiento interactivo que puede ser incorporada dentro de plantillas a través de bloques `[[LLM: instructions]]`. Este componente ofrece 10 acciones estructuradas de lluvia de ideas, capacidades de revisión sección por sección y flujos de trabajo de mejora iterativa para mejorar la calidad del contenido.
...
YARD incluye una capa de personalización a través del archivo `technical-preferences.md` en `yard-core/data/`. Este archivo sirve como un perfil técnico persistente que influye en el comportamiento de los agentes en todos los proyectos.
...
- **Para IDEs**: Los usuarios interactúan con los agentes directamente a través de sus archivos markdown en `yard-core/agents/`. La integración del IDE (para Cursor, Claude Code, etc.) sabe cómo llamar a estos agentes.
...
## 5. Flujos de Trabajo de YARD

### 5.1. El Flujo de Trabajo de Planificación

Antes de que comience el desarrollo, YARD sigue un flujo de trabajo de planificación estructurado que establece la base para la ejecución exitosa del proyecto:
...
**Orquestación del Flujo de Trabajo**: El agente `yard-orchestrator` usa estas definiciones de flujo de trabajo para guiar a los usuarios a través del proceso completo, asegurando transiciones apropiadas entre las fases de planificación (UI web) y desarrollo (IDE).

### 5.2. El Ciclo Central de Desarrollo

Una vez que las fases iniciales de planificación y arquitectura están completas, el proyecto pasa a un flujo de trabajo de desarrollo cíclico, como se detalla en `yard-kb.md`. Esto asegura un proceso de implementación constante, secuencial y con control de calidad.
