...
Todo el ecosistema del Método AIOX está diseñado alrededor del directorio `yard-core` instalado, que actúa como el cerebro de la operación. El directorio `tools` proporciona los medios para procesar y empaquetar este cerebro para diferentes entornos.
...
AIOX incluye una capa de personalización a través del archivo `technical-preferences.md` en `yard-core/data/`. Este archivo sirve como un perfil técnico persistente que influye en el comportamiento de los agentes en todos los proyectos.
