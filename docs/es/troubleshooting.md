...
# Guia de Solucion de Problemas de YARD-Framework

> 🌐 [EN](../troubleshooting.md) | [PT](../pt/troubleshooting.md) | **ES**

---

Esta guia completa te ayuda a diagnosticar y resolver problemas comunes con YARD-Framework.
...
# Salida detallada
npx yard-core doctor --verbose

# Verificar componente especifico
npx yard-core doctor --component memory-layer
```
...
npm exec yard-core init my-project
```
...
ls -la .yard/config.json

# Validar configuracion
npx yard-core doctor --component config
...
```javascript
// Editar .yard/memory-config.json
{
...
```
Error: "yard-core" cannot be opened
```
...
xattr -d com.apple.quarantine /usr/local/bin/yard-core
```
...
npx yard-core doctor --report diagnostic.json
...
```bash
# Analizar perfil
*debug analyze-profile .yard/profiles/latest.cpuprofile
```
...
1. **Ejecutar diagnosticos:**
   ```bash
   npx yard-core doctor --verbose > diagnostic.log
   ```
...
3. **Verificar issues existentes:**
   ...
   - [Discussions](https://github.com/yard-core/yard-core/discussions)
...
- **Stack Overflow**: Etiqueta preguntas con `yard-core`
