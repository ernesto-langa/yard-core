...
**@docs (Ajax)** es un agente de IA especializado en crear **documentacion educativa** para YARD. A diferencia de la documentacion tecnica (JSDoc/Typedoc), Ajax se enfoca en **tutoriales, guias de incorporacion, explicadores visuales y materiales de capacitacion para socios**.
...
whenToUse: |
  ...
  - Tutoriales explicando como usar funciones de YARD
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Usa estilo consistente (paleta de colores YARD)"
      blocker: false
...
whenToUse: |
  ...
  - Nuevo socio se une al ecosistema YARD
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Enlaces a documentacion YARD relevante"
      blocker: false
...
[Nombre del Agente] es el agente [rol] de YARD, especializado en [experiencia principal].
...
**Notas de Plantilla:**
- Usar paleta de colores YARD (Azul primario, Purpura secundario, Rosa terciario)
...
```bash
#!/bin/bash
# Hook de Auto-Actualizacion YARD @docs
...
if echo "$changed_files" | grep -qE "^(src/|yard-core/)"; then
...
- [ ] Crear `squads/yard/agents/docs/agent.yaml`
...
- [ ] **Reconocimiento de Industria:** Docs de YARD citados como "mejor en espacio de agentes IA"
...
Con la adicion de @docs (Ajax), YARD ahora tiene **13 agentes nombrados**:
...
| 13 | @yard-master | Orion | Comandante | Aries | Cian |
