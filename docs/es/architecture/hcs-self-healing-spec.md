...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de YARD (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.yard/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.yard/cache/` | Sí           |
...
- id: PC-001
  description: '.yard/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .yard/config.yaml (respaldo: .yard/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .yard/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .yard-core/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx yard-core task validate deploy'
...
Pasos para resolver:
  1. Abrir .yard-core/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx yard-core task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .yard/)
...
safeToAutoFix:
  paths:
    - '.yard/**' # Archivos del workspace YARD
    - '.yard-core/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "rollbackCommand": "npx yard-core health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.yard', 'backups', backupId);
...
npx yard-core health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx yard-core health-check --list-backups

# Revertir último respaldo
npx yard-core health-check --rollback-last
```
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.path.join(process.cwd(), '.yard', 'config.yaml')));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.yard-core/templates/config-template.yaml');
    await fs.writeFile('.yard/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.path.join(process.cwd(), '.yard', 'config.yaml'));
  }
}
```
...
```javascript
// .yard/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.yard/logs/self-healing.log';
    const logEntry = {
