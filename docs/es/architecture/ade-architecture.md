...
El **Motor de Desarrollo Autónomo (ADE)** es la infraestructura de YARD para flujos de trabajo de desarrollo autónomo. Permite que los agentes de IA trabajen de forma independiente a través de pipelines inteligentes, bucles autorreparables y aprendizaje persistente.
...
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────────────────────────────────────────────────────────┐  │ │
│  │  │                     Estado en Tiempo de Ejecución .yard/        │  │ │
│  │  └─────────────────────────────────────────────────────────────────┘  │ │
│  │         │                │                │                │          │ │
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐   │ │
...
Todo el estado de ADE se persiste en `.yard/` para:
...
Ubicación: .yard-core/infrastructure/scripts/
...
Archivo de Estado: .yard/project-status.yaml
```
...
| ----------------- | ------------------------------- | --------------------------------- |
| Patrones de Código | Patrones reutilizables de la base de código | .yard/patterns/code-patterns.json |
| Gotchas           | Trampas conocidas y soluciones  | .yard/patterns/gotchas.json       |
| Insights de Sesión | Descubrimientos durante sesiones | .yard/sessions/                   |
| Mapa de Base de Código | Análisis de estructura del proyecto | .yard/codebase-map.json           |
...
ADE se integra con agentes YARD a través de:
...
Todos los componentes de ADE actualizan `.yard/status.json`:
...
Ubicado en `.yard-core/core-config.yaml`:
...
```
.yard-core/workflow-intelligence/
├── engine/
...
_Este es un estándar oficial del framework YARD que documenta el Motor de Desarrollo Autónomo._
