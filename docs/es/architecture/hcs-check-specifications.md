...
| ------ | --------------------------- | --------- | ----- | ----- | ------- | --------------------------------------------------- |
| PC-001 | Config existe               | CRITICAL  | 1     | quick | 100ms   | `.yard/config.yaml` existe y es YAML válido         |
...
| PC-006 | Sin archivos huérfanos      | LOW       | 3     | full  | 5s      | Todos archivos en `.yard-core/` están referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    ...
    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.yard/config.yaml no encontrado',
...
Los usuarios pueden definir verificaciones específicas del proyecto en `.yard/custom-checks.yaml`:
