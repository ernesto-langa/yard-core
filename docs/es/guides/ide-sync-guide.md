...
Sincroniza agentes, tasks, workflows y checklists de YARD a traves de multiples configuraciones de IDE.

## Vision General

La task `*command` automatiza la sincronizacion de componentes YARD a todos los directorios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operaciones de copia manuales.
...
Editar `.yard-sync.yaml` para habilitar tus IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Leer configuracion .yard-sync.yaml               │
...
```
tu-proyecto/
├── .yard-sync.yaml           # Configuracion de sincronizacion
...
**Solucion**: Agregar el alias a `.yard-sync.yaml`:
...
- [Arquitectura YARD](../core-architecture.md)
