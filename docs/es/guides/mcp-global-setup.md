...
> Configura servidores MCP (Model Context Protocol) globales para YARD-Framework.
...
- `~/.yard/mcp/cache/` - Caché de respuestas
- `~/.yard/credentials/` - Almacenamiento seguro de credenciales
...
Las credenciales se almacenan en `~/.yard/credentials/` con un `.gitignore` para prevenir commits accidentales.
...
_YARD-Framework v4 Guía de Configuración Global de MCP_
