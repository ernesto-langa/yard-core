...
El Sistema de Saludo Contextual es una mejora de UX que hace que los saludos de los agentes YARD sean inteligentes y adaptativos, mostrando información y comandos relevantes basados en el contexto de la sesión.

## Lo Que Se Ha Implementado

### Componentes Core (Story 6.1.2.5)

1. **ContextDetector** (`.yard-core/core/session/context-detector.js`)
...
2. **GitConfigDetector** (`.yard-core/infrastructure/scripts/git-config-detector.js`)
   - Detecta configuración de git del proyecto
   - Caché con TTL de 5 minutos
   - Protección de timeout de 1000ms

3. **GreetingBuilder** (`.yard-core/development/scripts/greeting-builder.js`)
...
4. **WorkflowNavigator** (`.yard-core/development/scripts/workflow-navigator.js`)
...
5. **Workflow Patterns** (`.yard-core/data/workflow-patterns.yaml`)
...
```bash
node .yard-core/development/scripts/test-greeting-system.js
```
...
```javascript
const GreetingBuilder = require('./.yard-core/development/scripts/greeting-builder');
...
### Scripts Core
- `.yard-core/core/session/context-detector.js` - Detección de tipo de sesión
- `.yard-core/infrastructure/scripts/git-config-detector.js` - Detección de git config
- `.yard-core/development/scripts/greeting-builder.js` - Construcción del saludo
- `.yard-core/development/scripts/workflow-navigator.js` - Navegación de workflow
- `.yard-core/development/scripts/agent-exit-hooks.js` - Hooks de salida (para persistencia)

### Archivos de Datos
- `.yard-core/data/workflow-patterns.yaml` - Definiciones de workflows
...
### Configuración
- `.yard-core/core-config.yaml` - Configuración global (secciones git + agentIdentity)

### Agentes (Actualizados)
- `.yard-core/agents/dev.md` - Metadatos de visibilidad de comandos
- `.yard-core/agents/po.md` - Metadatos de visibilidad de comandos
- `.yard-core/agents/*.md` - 9 agentes restantes (actualización pendiente)
...
R: Usa el script de prueba: `node .yard-core/development/scripts/test-greeting-system.js`
