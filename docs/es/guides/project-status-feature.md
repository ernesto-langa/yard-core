...
La funcionalidad de Estado del Proyecto muestra automáticamente tu contexto de trabajo actual cuando cualquier agente YARD se activa. Esto incluye:
...
3. Crea el archivo de caché `.yard/project-status.yaml`
...
1. Editar `.yard-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Ubicación: `.yard-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .yard/project-status.yaml  # Ubicación del archivo de caché
...
1. **Verifica caché** - Busca `.yard/project-status.yaml`
...
- **Ubicación del Caché:** `.yard/project-status.yaml`
...
Los 11 agentes YARD muestran estado del proyecto:
...
3. ¿Existe `.yard-core/infrastructure/scripts/project-status-loader.js`?
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.yard-core/infrastructure/scripts/project-status-loader.js');
...
const { clearCache } = require('./.yard-core/infrastructure/scripts/project-status-loader.js');
await clearCache();
```
...
```bash
# Remove script
rm .yard-core/infrastructure/scripts/project-status-loader.js

# Remove task
rm .yard-core/tasks/init-project-status.md

# Remove cache
rm .yard/project-status.yaml

# Remove tests
rm .yard-core/infrastructure/scripts/__tests__/project-status-loader.test.js
...
R: Sí, todos los agentes usan el mismo archivo de caché (`.yard/project-status.yaml`).
...
- **Config:** `.yard-core/core-config.yaml` (sección projectStatus)
- **Script:** `.yard-core/infrastructure/scripts/project-status-loader.js`
- **Init Task:** `.yard-core/tasks/init-project-status.md`
- **Tests:** `.yard-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
