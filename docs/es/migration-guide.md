...
# Guia de Actualizacion de YARD-Framework

> 🌐 [EN](../migration-guide.md) | [PT](../pt/migration-guide.md) | **ES**

---

Esta guia te ayuda a actualizar entre versiones de YARD-Framework.
...
**YARD-Framework v4.2.11** (Version Actual del Repositorio)
...
tar -czf yard-backup-$(date +%Y%m%d-%H%M%S).tar.gz \
  --exclude=node_modules \
  --exclude=.git \
  .

# Mover a ubicacion segura
mv yard-backup-*.tar.gz ../backups/
```
...
cp .yard-core/config.json ../backups/config-backup.json

# Guardar componentes personalizados
cp -r .yard-core/agents/custom ../backups/custom-agents/
cp -r .yard-core/tasks/custom ../backups/custom-tasks/
```
...
npm list yard-core/core > ../backups/version-info.txt

# Listar archivos personalizados
find .yard-core -name "*.custom.*" -type f > ../backups/custom-files.txt
```
...
cd your-project
npm update yard-core/core

# Reinstalar dependencias
npm install

# Verificar actualizacion
npm list yard-core/core
```
...
tar -xzf backups/yard-backup-YYYYMMDD-HHMMSS.tar.gz
...
cp ../backups/config-backup.json .yard-core/config.json
cp -r ../backups/custom-agents/* .yard-core/agents/custom/
...
npm install -g yard-core@latest --verbose
...
1. **Verificar Logs**: Revisar `.yard-core/logs/upgrade.log`
2. **GitHub Issues**: [github.com/ernesto-langa/yard-core/issues](https://github.com/ernesto-langa/yard-core/issues)
