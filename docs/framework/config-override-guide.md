...
```
L1 Framework (.yard-core/framework-config.yaml)   — Read-only, shipped with npm
  ↓ deep merge
L2 Project (.yard-core/project-config.yaml)        — Team-shared, committed
  ↓ deep merge
...
L4 Local (.yard-core/local-config.yaml)            — Machine-specific, gitignored
  ↓ deep merge
L5 User (~/.yard/user-config.yaml)                 — Cross-project user prefs
```
...
| `resource_locations.tasks_dir` | string | `.yard-core/development/tasks` | L2+ | Task definitions path |
| `resource_locations.templates_dir` | string | `.yard-core/development/templates` | L2+ | Templates path |
| `resource_locations.checklists_dir` | string | `.yard-core/development/checklists` | L2+ | Checklists path |
| `resource_locations.tools_dir` | string | `.yard-core/tools` | L2+ | Tools path |
| `resource_locations.scripts.*` | string | various | L2+ | Script paths by category |
| `resource_locations.data_dir` | string | `.yard-core/data` | L2+ | Data directory |
...
### L5 User Keys (~/.yard/user-config.yaml)
...
Schema files: `.yard-core/core/config/schemas/`
