# CLAUDE.md - YARD-Framework

Este arquivo configura o comportamento do Claude Code ao trabalhar neste repositório.

---

<!-- FRAMEWORK-OWNED: Generated by YARD installer, do not customize -->
## Constitution
...
The installer writes this automatically during `npx yard-core install`. No language config in `core-config.yaml`.
...
O YARD-Framework segue uma hierarquia clara de prioridades que deve guiar **TODAS** as decisões:
...
├── bin/                     # CLI executables (yard-init.js, yard.js)
├── docs/                    # Documentação
...
|--------|----------------------|
| `@dev` | `packages/`, `.yard-core/core/`, `bin/` |
...
- Agent memory persists in `.yard-core/development/agents/{id}/MEMORY.md`
...
### YARD
```bash
npx yard-core install       # Instalar YARD
npx yard-core doctor        # Diagnóstico do sistema
npx yard-core info          # Informações do sistema
```
...
Prefer native Claude Code tools over MCP for common operations. The tool-registry at `.yard-core/data/tool-registry.yaml` defines the 3-Tier Tool Mesh:
...
**Tool Examples (TOK-4B):** Concrete input examples improve tool selection accuracy by +18pp. See `.claude/rules/tool-examples.md` for examples of top-10 tools. Full registry: `.yard-core/data/mcp-tool-examples.yaml`. ADR-5: examples for always-loaded tools only, search for deferred tools.

**Runtime capabilities:** `.yard/runtime-capabilities.json` (generated by capability-detection.js)
...
*YARD-Framework Claude Code Configuration v4.0*
*CLI First | Observability Second | UI Third*
