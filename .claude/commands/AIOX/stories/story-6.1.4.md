...
- STEP 3: |
    ...
    1. Execute: node .yard-core/scripts/generate-greeting.js {agent-id}
...
Add to `.yard-core/scripts/agent-config-loader.js`:
...
async loadAgentDefinition(options = {}) {
  ...
  // Load from file
  const agentPath = path.join(process.cwd(), '.yard-core', 'agents', `${this.agentId}.md`);
...
1. Add deprecation warning to `.yard-core/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .yard-core/
grep -r "from.*config-loader" .yard-core/
```
...
1. `.yard-core/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.yard-core/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.yard-core/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.yard-core/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .yard-core/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .yard-core/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .yard-core/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .yard-core/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .yard-core/scripts/batch-integrate-greeting-builder.js ] && rm .yard-core/scripts/batch-integrate-greeting-builder.js
   [ -f .yard-core/scripts/update-activation-instructions.js ] && rm .yard-core/scripts/update-activation-instructions.js
   ```
...
Modify `.yard-core/scripts/greeting-builder.js`:
...
Create `.yard-core/scripts/generate-greeting.js`:
...
async function generateGreeting(agentId) {
  ...
  try {
    // Load core config
    const coreConfigPath = path.join(process.cwd(), '.yard-core', 'core-config.yaml');
...
- STEP 3: |
    ...
    1. Execute: node .yard-core/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
3. **Direct Script Test:**
   ...
   node .yard-core/scripts/generate-greeting.js qa
...
4. **Error Handling Test:**
   ...
   node .yard-core/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .yard-core/scripts/generate-greeting.js qa
...
Create script `.yard/temp-update-remaining-agents.js`:
...
Create `.yard-core/scripts/command-execution-hook.js`:
...
Create `.yard-core/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.yard-core/scripts/generate-greeting');
...
- `.yard-core/scripts/generate-greeting.js` (Unified greeting generator)
- `.yard-core/scripts/command-execution-hook.js` (Session update hook)
- `.yard-core/docs/session-update-pattern.md` (Documentation)
- `tests/unit/generate-greeting.test.js` (Unit tests)
- `tests/integration/greeting-system-integration.test.js` (Integration tests)

### Files Modified

- `.yard-core/scripts/agent-config-loader.js` (Expanded with `loadAgentDefinition()` and `loadComplete()`)
- `.yard-core/scripts/greeting-builder.js` (Modified to accept pre-loaded context)
- `.yard-core/scripts/config-loader.js` (Deprecated warning added)
- `.yard-core/agents/qa.md` (Pilot - STEP 3 updated first)
- `.yard-core/agents/*.md` (Remaining 10 agents - STEP 3 updated after QA validation)

### Files Deleted

- `.yard-core/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.yard-core/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.yard-core/scripts/update-activation-instructions.js` (Temporary migration script)
- `.yard-core/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.yard-core/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.yard-core/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.yard-core/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.yard-core/scripts/context-detector.js` (Used by greeting-builder.js)
- `.yard-core/scripts/workflow-navigator.js` (Used by greeting-builder.js)
