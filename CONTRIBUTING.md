...
```
.yard-core/development/agents/your-agent.md
```
...
```
.yard-core/development/tasks/your-task.md
```
...
- [Squad Discussions](https://github.com/ernesto-langa/yard-core/discussions/categories/ideas) - Share ideas
...
Critical paths require approval from `@ernesto-langa` or `@ernesto-langa` (maintainers):

| Path | Why |
|------|-----|
| `.yard-core/core/orchestration/` | Orchestration layer (MasterOrchestrator, GateEvaluator) |
| `.yard-core/core/execution/` | Execution engine (WaveExecutor, ParallelExecutor) |
| `packages/` | Installer, CLI, shared libraries |
| `.github/` | CI/CD workflows, branch protection |
| `.yard-core/core-config.yaml` | Framework configuration |

All other paths require review from any maintainer (`@ernesto-langa` or `@ernesto-langa`).
...
- **Discussions:** [Start a discussion](https://github.com/ernesto-langa/yard-core/discussions)
...
git clone https://github.com/<your-fork>/yard-core.git
cd yard-core
...
git clone --recurse-submodules https://github.com/ernesto-langa/yard-core.git

# Or add to existing clone
git submodule update --init pro
```

**Push order:** Always push `pro/` changes first, then `yard-core`.
