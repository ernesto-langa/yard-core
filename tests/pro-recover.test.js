...
const { maskEmail, openBrowser, promptEmail, recoverLicense, RECOVERY_URL, RECOVERY_MESSAGE } = require('../packages/yard-pro-cli/src/recover');
...
describe('CLI alias reset-password', () => {
  test('yard-pro.js switch handles reset-password same as recover', () => {
    // Verify the CLI entry point has reset-password as a case that calls recoverLicense
    const cliSource = require('fs').readFileSync(
      require('path').join(__dirname, '../packages/yard-pro-cli/bin/yard-pro.js'),
      'utf-8'
    );
