...
const {
  ...
  findLatestBackup,
  listBackups,
} = require('../../.yard-core/cli/commands/migrate/backup');
...
describe('Migration Backup Module', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `yard-backup-test-${testId}`);
  ...
  describe('createBackupDirName', () => {
    it('should create backup directory name with date format', () => {
      ...
      expect(name).toMatch(/^\.yard-backup-\d{4}-\d{2}-\d{2}$/);
    });

    it('should use current date', () => {
      const name = createBackupDirName();
      const today = new Date().toISOString().split('T')[0];
      expect(name).toBe(`.yard-backup-${today}`);
    });
  });
  ...
  describe('createBackup', () => {
    ...
    it('should fail if .yard-core does not exist', async () => {
      await expect(createBackup(testDir)).rejects.toThrow(/No .yard-core directory/);
    });
  ...
  describe('verifyBackup', () => {
    ...
    it('should detect corrupted files', async () => {
      ...
      const backedUpFile = path.join(backupResult.backupDir, '.yard-core', 'test.js');
  ...
  describe('findLatestBackup', () => {
    it('should find the most recent backup', async () => {
      ...
      expect(latest.name).toMatch(/^\.yard-backup-/);
    });
