...
describe('Mode Detector', () => {
  ...
  describe('detectInstallationMode', () => {
    ...
    it('should detect FRAMEWORK_DEV mode for yard-core repository', () => {
      // Create .yard-core directory
      fs.mkdirSync(path.join(tempDir, '.yard-core'));

      // Create package.json with yard-core name
      fs.writeFileSync(
  ...
  describe('isAioxCoreRepository', () => {
    ...
    it('should return false for generic monorepo with workspaces pattern only', () => {
      // Generic monorepos should NOT be detected as yard-core
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
  ...
  describe('validateModeSelection', () => {
    ...
    it('should warn when selecting framework-dev for non-yard-core repo', () => {
      ...
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('yard-core repository');
    });
