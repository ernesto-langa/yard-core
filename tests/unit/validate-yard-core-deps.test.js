...
const VALIDATOR_PATH = path.join(PROJECT_ROOT, 'scripts', 'validate-yard-core-deps.js');

describe('validate-yard-core-deps.js (INS-4.12)', () => {
  ...
  test('.yard-core/package.json includes fast-glob', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.yard-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fast-glob');
  });

  test('.yard-core/package.json includes fs-extra', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.yard-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fs-extra');
  });

  test('.yard-core/package.json includes semver', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.yard-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('semver');
  });

  test('.yard-core/package.json includes ajv', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.yard-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('ajv');
  });

  test('.yard-core/package.json includes tar', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.yard-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
describe('validate-publish.js dependency check (INS-4.12)', () => {
  test('validate-publish includes dependency completeness check', () => {
    ...
    expect(content).toContain('validate-yard-core-deps');
  });
});
