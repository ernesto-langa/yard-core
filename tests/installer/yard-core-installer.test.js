...
const {
  generateFileHashes,
  generateVersionJson,
} = require('../../packages/installer/src/installer/yard-core-installer');

describe('Yard Core Installer - Version Tracking', () => {
  let tempDir;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'yard-installer-test-'));
    await fs.ensureDir(path.join(tempDir, '.yard-core'));
  });
  ...
  describe('generateFileHashes', () => {
    it('should generate hashes for installed files', async () => {
      const yardCoreDir = path.join(tempDir, '.yard-core');

      // Create test files
      await fs.writeFile(path.join(yardCoreDir, 'test1.md'), '# Test File 1');
      await fs.writeFile(path.join(yardCoreDir, 'test2.md'), '# Test File 2');
      await fs.ensureDir(path.join(yardCoreDir, 'agents'));
      await fs.writeFile(path.join(yardCoreDir, 'agents', 'dev.md'), '# Dev Agent');

      const installedFiles = ['test1.md', 'test2.md', 'agents/dev.md'];
      const hashes = await generateFileHashes(yardCoreDir, installedFiles);
    ...
    it('should skip non-existent files', async () => {
      ...
      await fs.writeFile(path.join(yardCoreDir, 'exists.md'), '# Exists');
    ...
    it('should skip directories', async () => {
      ...
      await fs.ensureDir(path.join(yardCoreDir, 'agents'));
      await fs.writeFile(path.join(yardCoreDir, 'file.md'), '# File');
    ...
    it('should generate consistent hashes for same content', async () => {
      ...
      await fs.writeFile(path.join(yardCoreDir, 'file1.md'), 'Same content');
      await fs.writeFile(path.join(yardCoreDir, 'file2.md'), 'Same content');
    ...
    it('should generate different hashes for different content', async () => {
      ...
      await fs.writeFile(path.join(yardCoreDir, 'file1.md'), 'Content A');
      await fs.writeFile(path.join(yardCoreDir, 'file2.md'), 'Content B');
  ...
  describe('generateVersionJson', () => {
    it('should create version.json with correct structure', async () => {
      ...
      // Create test files
      await fs.writeFile(path.join(yardCoreDir, 'test.md'), '# Test');

      const result = await generateVersionJson({
        targetYardCore: yardCoreDir,
        version: '1.2.0',
    ...
    it('should write version.json to disk', async () => {
      ...
      await fs.writeFile(path.join(yardCoreDir, 'agent.md'), '# Agent');
      ...
      const versionJsonPath = path.join(yardCoreDir, 'version.json');
    ...
    it('should include file hashes in version.json', async () => {
      ...
      await fs.ensureDir(path.join(yardCoreDir, 'agents'));
      await fs.writeFile(path.join(yardCoreDir, 'agents', 'dev.md'), '# Dev');
      await fs.writeFile(path.join(yardCoreDir, 'config.yaml'), 'key: value');
