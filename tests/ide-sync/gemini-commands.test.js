...
const {
  ...
  syncGeminiCommands,
} = require('../../.yard-core/infrastructure/scripts/ide-sync/gemini-commands');
...
describe('gemini command sync', () => {
  ...
  it('normalizes command slugs and menu names', () => {
    expect(commandSlugForAgent('yard-master')).toBe('master');
    expect(commandSlugForAgent('dev')).toBe('dev');
    expect(menuCommandName('yard-master')).toBe('/yard-master');
    expect(menuCommandName('dev')).toBe('/yard-dev');
  });
  ...
  it('builds menu + one command per agent', () => {
    ...
    expect(files.find((f) => f.filename === 'yard-menu.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'yard-dev.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'yard-architect.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'yard-qa.toml')).toBeDefined();
  ...
  it('derives command description from agent title and whenToUse', () => {
    ...
    const dev = files.find((f) => f.filename === 'yard-dev.toml');
  ...
  it('falls back to generic description when metadata is missing', () => {
    ...
    const dev = files.find((f) => f.filename === 'yard-dev.toml');
    expect(dev.content).toContain('description = "Ativar agente YARD dev"');
  });
  ...
  it('writes command files to .gemini/commands', () => {
    ...
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'yard-menu.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'yard-dev.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'yard-qa.toml'))).toBe(true);
  });
});
