...
const { commandValidate } = require('../../.yard-core/infrastructure/scripts/ide-sync/index');
const { parseAllAgents } = require('../../.yard-core/infrastructure/scripts/ide-sync/agent-parser');
const claudeTransformer = require('../../.yard-core/infrastructure/scripts/ide-sync/transformers/claude-code');
const { syncGeminiCommands } = require('../../.yard-core/infrastructure/scripts/ide-sync/gemini-commands');
...
describe('ide-sync commandValidate --ide filter', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.ensureDir(path.join(tmpRoot, '.yard-core'));
    await fs.writeFile(
      path.join(tmpRoot, '.yard-core', 'core-config.yaml'),
      [
        'ideSync:',
        '  enabled: true',
        '  source: .yard-core/development/agents',
        '  targets:',
        '    claude-code:',
        '      enabled: true',
    ...
    await fs.copy(
      path.join(previousCwd, '.yard-core', 'development', 'agents'),
