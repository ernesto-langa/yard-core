...
const { RegistrySyncer, inferRole, ROLE_MAP } = require('../../.yard-core/core/code-intel/registry-syncer');
    ...
    RegistryLoader: jest.fn().mockImplementation(() => ({
      load: jest.fn().mockReturnValue({
        ...
        entities: {
          tasks: {
            'dev-develop-story': {
              path: '.yard-core/development/tasks/dev-develop-story.md',
              type: 'task',
            ...
            'create-next-story': {
              path: '.yard-core/development/tasks/create-next-story.md',
              type: 'task',
          ...
          scripts: {
            'greeting-builder': {
              path: '.yard-core/development/scripts/greeting-builder.js',
              type: 'script',
...
function createMockClient(overrides = {}) {
  return {
    findReferences: jest.fn().mockResolvedValue([
      { file: '.yard-core/development/tasks/create-next-story.md' },
    ]),
...
describe('RegistrySyncer', () => {
  ...
  describe('usedBy population (T2 — AC2)', () => {
    ...
    it('should deduplicate usedBy entries', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.yard-core/development/tasks/create-next-story.md' },
          { file: '.yard-core/development/tasks/create-next-story.md' }, // Duplicate
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
      ...
      const entities = {
        tasks: {
          'test-entity': {
            path: '.yard-core/development/tasks/test.md',
            usedBy: [],
            dependencies: [],
          },
        },
  ...
  describe('codeIntelMetadata schema (T4 — AC4)', () => {
    ...
    it('should set callerCount based on usedBy length', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.yard-core/development/tasks/create-next-story.md' },
          { file: '.yard-core/development/scripts/greeting-builder.js' },
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
    ...
    it('should set provider from active provider name', async () => {
      ...
      const entity = {
        id: 'test',
        category: 'tasks',
        data: { path: '.yard-core/development/tasks/test.md', usedBy: [], dependencies: [] },
      };
  ...
  describe('inferRole', () => {
    it('should infer role from path patterns', () => {
      expect(inferRole('.yard-core/development/tasks/dev.md')).toBe('task');
      expect(inferRole('.yard-core/development/agents/dev.md')).toBe('agent');
      expect(inferRole('.yard-core/development/workflows/sdc.yaml')).toBe('workflow');
      expect(inferRole('.yard-core/development/scripts/build.js')).toBe('script');
      expect(inferRole('.yard-core/core/utils/helper.js')).toBe('module');
      expect(inferRole('.yard-core/data/entity-registry.yaml')).toBe('config');
      expect(inferRole('.yard-core/product/templates/prd.yaml')).toBe('template');
    });
