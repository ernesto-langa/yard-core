...
const { resolveCodeIntel, formatAsXml, escapeXml, _resetForTesting } = require(
  path.join(__dirname, '..', '..', '.yard-core', 'core', 'code-intel', 'hook-runtime.js'),
);
...
describe('code-intel hook-runtime', () => {
  ...
  describe('resolveCodeIntel', () => {
    ...
    it('handles absolute paths by normalizing to relative', async () => {
      const absPath = path.join(PROJECT_ROOT, '.yard-core', 'development', 'tasks', 'create-next-story.md');
  ...
  describe('formatAsXml', () => {
    ...
    it('generates valid XML with entity data', () => {
      const intel = {
        entity: {
          file: '.yard-core/core/some-module.js',
      ...
      expect(xml).toContain('<path>.yard-core/core/some-module.js</path>');
