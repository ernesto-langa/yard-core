...
jest.mock('../../.yard-core/core/execution/build-state-manager', () => ({
  BuildStateManager: jest.fn().mockImplementation(() => ({ ...mockStateInstance })),
}));

// Mock optional dependencies to not load
jest.mock('../../.yard-core/infrastructure/scripts/recovery-tracker', () => {
  throw new Error('not available');
});
jest.mock('../../.yard-core/infrastructure/scripts/worktree-manager', () => {
  throw new Error('not available');
});
...
const {
  ...
  SubtaskResult,
  DEFAULT_CONFIG,
} = require('../../.yard-core/core/execution/autonomous-build-loop');

const { BuildStateManager } = require('../../.yard-core/core/execution/build-state-manager');
