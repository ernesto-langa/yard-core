...
const { WorkflowExecutor } = require('../../../.yard-core/core/orchestration/workflow-executor');
...
describe('WorkflowExecutor Callbacks (Story 12.6)', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.ensureDir(path.join(tempDir, '.yard-core/development/workflows'));
...
`;
    await fs.writeFile(
      path.join(tempDir, '.yard-core/development/workflows/development-cycle.yaml'),
      workflowContent,
    );
