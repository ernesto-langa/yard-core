...
const ContextManager = require('../../../.yard-core/core/orchestration/context-manager');
...
describe('ContextManager', () => {
  ...
  describe('constructor', () => {
    ...
    test('calcula stateDir como .yard/workflow-state dentro do projectRoot', () => {
      expect(manager.stateDir).toBe(path.join(PROJECT_ROOT, '.yard', 'workflow-state'));
    });

    test('calcula statePath com o workflowId no nome do arquivo', () => {
      expect(manager.statePath).toBe(
        path.join(PROJECT_ROOT, '.yard', 'workflow-state', `${WORKFLOW_ID}.json`),
      );
    });

    test('calcula handoffDir dentro do stateDir', () => {
      expect(manager.handoffDir).toBe(
        path.join(PROJECT_ROOT, '.yard', 'workflow-state', 'handoffs'),
      );
    });

    test('calcula confidenceDir dentro do stateDir', () => {
      expect(manager.confidenceDir).toBe(
        path.join(PROJECT_ROOT, '.yard', 'workflow-state', 'confidence'),
      );
    });
