...
const { createBackup, verifyBackup } = require('../../.yard-core/cli/commands/migrate/backup');
const { detectV2Structure, analyzeMigrationPlan } = require('../../.yard-core/cli/commands/migrate/analyze');
const { executeMigration } = require('../../.yard-core/cli/commands/migrate/execute');
const { updateAllImports, verifyImports } = require('../../.yard-core/cli/commands/migrate/update-imports');
const { validateStructure } = require('../../.yard-core/cli/commands/migrate/validate');
const { executeRollback, canRollback } = require('../../.yard-core/cli/commands/migrate/rollback');
...
describe('Full Migration Integration', () => {
  ...
  describe('MIG-04: Core Migrated', () => {
    it('should migrate core module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'core', 'registry', 'index.js'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'core', 'utils', 'helpers.js'))).toBe(true);
    });
  });
  ...
  describe('MIG-05: Dev Migrated', () => {
    it('should migrate development module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'development', 'agents', 'dev.md'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'development', 'tasks', 'build.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-08: Rollback Works', () => {
    it('should rollback to original v2.0 state', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'development'))).toBe(true);
      ...
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'agents', 'dev.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-09: Dry Run', () => {
    it('should show plan without making changes in dry run mode', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'development'))).toBe(false);
      expect(fs.existsSync(path.join(testDir, '.yard-core', 'core'))).toBe(false);
    });
  });
  ...
  describe('MIG-11: Conflict Detection', () => {
    it('should detect existing v2.1 directories as conflicts', async () => {
      ...
      // Add a conflict
      await fs.promises.mkdir(path.join(testDir, '.yard-core', 'core'), { recursive: true });
  ...
  describe('MIG-12: Permissions', () => {
    it('should preserve file permissions during migration', async () => {
      ...
      const originalFile = path.join(testDir, '.yard-core', 'cli', 'index.js');
      const originalStats = await fs.promises.stat(originalFile);

      const plan = await analyzeMigrationPlan(testDir);
      await executeMigration(plan, { cleanupOriginals: false });

      const migratedFile = path.join(testDir, '.yard-core', 'product', 'cli', 'index.js');
