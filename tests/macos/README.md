# macOS Testing Suite for YARD
**Story 1.10b - macOS Testing & Validation**

Comprehensive test automation framework for validating YARD installation on macOS (Intel and Apple Silicon).
...
```bash
./run-all-tests.sh
# View report at: /tmp/yard-macos-test-report-*.txt
```
...
cd ~/path/to/yard-core/tests/macos
./run-all-tests.sh
```
...
2. **Test Logs:**
   - Attach all `/tmp/yard-test-*.log` files
