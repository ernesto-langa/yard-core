...
const PKG_DIR = path.resolve(__dirname, '../../../packages/yard-install');
...
describe('Integration - Task 8.3: Local NPX Execution', () => {
  ...
  describe('Package Structure Validation', () => {
    it('should have valid package.json', () => {
      ...
      // Then
      expect(pkgJson.name).toBe('@ernesto-langa/yard-install');
      expect(pkgJson.bin).toBeDefined();
      expect(pkgJson.bin['yard-install']).toBe('./bin/yard-install.js');
    ...
    it('should have executable shebang in bin files', () => {
      // Given
      const binFiles = ['bin/yard-install.js', 'bin/edmcp.js'];
  ...
  describe('CLI Execution Tests', () => {
    it('should execute yard-install --version via node', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/yard-install.js');
    ...
    it('should show help for yard-install', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/yard-install.js');
      ...
      // Then
      expect(result).toContain('yard-install');
  ...
  (runNpxIntegration ? describe : describe.skip)('NPX Local Execution Simulation', () => {
    it('should execute via npm exec (simulates npx .)', () => {
      ...
      // When
      const result = execSync('npm exec -- yard-install --version', {
        cwd: PKG_DIR,
        encoding: 'utf8',
        timeout: 90000,
