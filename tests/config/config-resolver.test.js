...
const {
  ...
  LEVELS,
  VALID_USER_PROFILES,
} = require('../../.yard-core/core/config/config-resolver');
...
describe('config-resolver', () => {
  ...
  describe('constants', () => {
    ...
    test('CONFIG_FILES.user points to ~/.yard/user-config.yaml', () => {
      const expected = path.join(os.homedir(), '.yard', 'user-config.yaml');
  ...
  describe('isLegacyMode', () => {
    test('returns true when core-config.yaml exists but framework-config.yaml does not', () => {
      const tmpDir = createTempProject({
        '.yard-core/core-config.yaml': 'project:\n  name: legacy\n',
      });
  ...
  describe('loadLayeredConfig — 4-level resolution', () => {
    ...
    test('merges L1 + L2 + L4 correctly', () => {
      const tmpDir = createTempProject({
        '.yard-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.yard-core/project-config.yaml': { fixture: 'project-config.yaml' },
        '.yard-core/local-config.yaml': { fixture: 'local-config.yaml' },
      });
    ...
    test('lints L1 and L2 for env patterns', () => {
      const tmpDir = createTempProject({
        '.yard-core/framework-config.yaml': 'url: "${API_URL}"\n',
        '.yard-core/project-config.yaml': 'name: "clean"\n',
      });
  ...
  describe('resolveConfig — main entry point', () => {
    ...
    test('interpolates env vars after merge', () => {
      const tmpDir = createTempProject({
        '.yard-core/framework-config.yaml': 'metadata:\n  name: "test"\n',
        '.yard-core/local-config.yaml': 'api_url: "${TEST_API_URL:-http://localhost}"\n',
      });
  ...
  describe('L5 User layer', () => {
    ...
    test('L5 overrides L4 values', () => {
      const tmpDir = createTempProject({
        '.yard-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.yard-core/local-config.yaml': 'user_profile: "advanced"\n',
      });
  ...
  describe('integration — L5 with full hierarchy', () => {
    ...
    test('resolveConfig returns user_profile from L5 overriding lower levels', () => {
      const tmpDir = createTempProject({
        '.yard-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.yard-core/project-config.yaml': 'user_profile: "advanced"\nproject:\n  name: test\n',
      });
