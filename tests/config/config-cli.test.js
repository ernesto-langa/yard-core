...
const { createConfigCommand } = require('../../.yard-core/cli/commands/config');
const { globalConfigCache } = require('../../.yard-core/core/config/config-cache');
...
describe('config CLI commands', () => {
  ...
  describe('yard config show', () => {
    test('shows resolved config as YAML', async () => {
      const tmpDir = createTempProject({
        ...
        '.yard-core/project-config.yaml': { fixture: 'project-config.yaml' },
      });
  ...
  describe('yard config migrate', () => {
    ...
    test('--dry-run shows preview without writing files', async () => {
      const tmpDir = createTempProject({
        '.yard-core/core-config.yaml': LEGACY_CONFIG,
      });
      ...
      try {
        ...
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'framework-config.yaml'))).toBe(false);
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'project-config.yaml'))).toBe(false);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
    ...
    test('full migration creates split files and backup', async () => {
      ...
      try {
        ...
        // Split files created
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'framework-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'project-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'local-config.yaml'))).toBe(true);

        // Backup created
        expect(fs.existsSync(path.join(tmpDir, '.yard-core', 'core-config.yaml.backup'))).toBe(true);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
  ...
  describe('yard config init-local', () => {
    test('creates local-config.yaml from template', async () => {
      ...
      try {
        ...
        const localConfig = path.join(tmpDir, '.yard-core', 'local-config.yaml');
    ...
    test('warns if local-config.yaml already exists', async () => {
      const tmpDir = createTempProject({
        '.yard-core/local-config.yaml.template': 'ide:\n  selected:\n    - vscode\n',
        '.yard-core/local-config.yaml': 'existing: true\n',
      });
