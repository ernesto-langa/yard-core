...
async function runWizard(options = {}) {
  ...
  if (!fs.existsSync(wizardPath)) {
    ...
    console.error('Please ensure Yard is installed correctly.');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
Yard v${packageJson.version}
AI-Orchestrated System for Full Stack Development

USAGE:
  npx yard-core@latest              # Run installation wizard
  npx yard-core@latest install      # Install in current project
  npx yard-core@latest init <name>  # Create new project
  npx yard-core@latest update       # Update to latest version
  npx yard-core@latest validate     # Validate installation integrity
  npx yard-core@latest info         # Show system info
  npx yard-core@latest doctor       # Run diagnostics
  npx yard-core@latest --version    # Show version
  npx yard-core@latest --version -d # Show detailed version info
  npx yard-core@latest --help       # Show this help

UPDATE:
  yard-core update                    # Update to latest version
  yard-core update --check            # Check for updates without applying
  yard-core update --dry-run          # Preview what would be updated
  yard-core update --force            # Force update even if up-to-date
  yard-core update --verbose          # Show detailed output

VALIDATION:
  yard-core validate                    # Validate installation integrity
  yard-core validate --repair           # Repair missing/corrupted files
  yard-core validate --repair --dry-run # Preview repairs
  yard-core validate --detailed         # Show detailed file list

CONFIGURATION:
  yard-core config show                       # Show resolved configuration
  yard-core config show --debug               # Show with source annotations
  yard-core config diff --levels L1,L2        # Compare config levels
  yard-core config migrate                    # Migrate monolithic to layered
  yard-core config validate                   # Validate config files
  yard-core config init-local                 # Create local-config.yaml

SERVICE DISCOVERY:
  yard-core workers search <query>            # Search for workers
  yard-core workers search "json" --category=data
  yard-core workers search "transform" --tags=etl,data
  yard-core workers search "api" --format=json
...
EXAMPLES:
  ...
  # Search for workers
  yard-core workers search "json csv"

For more information, visit: https://github.com/ernesto-langa/yard-core
`);
}
...
async function showVersion() {
  ...
  console.log(`Yard v${packageJson.version}`);
  console.log('Package: yard-core');

  // Check for local installation
  const localVersionPath = path.join(process.cwd(), '.yard-core', 'version.json');
  ...
  if (fs.existsSync(localVersionPath)) {
    try {
      ...
      if (versionInfo.version !== packageJson.version) {
        ...
        console.log('  Run \'npx yard-core update\' to update.');
      } else {
        console.log('\n✅ Up to date');
      }
    } catch (error) {
    ...
    console.log('  Run \'npx yard-core install\' to install Yard in this project.');
  }
}

// Helper: Show system info
function showInfo() {
  console.log('📊 Yard System Information\n');
  ...
  if (fs.existsSync(yardCoreDir)) {
    ...
    const componentBase = fs.existsSync(devDir) ? devDir : yardCoreDir;
    ...
    console.log('\n⚠️  Yard Core not found');
  }

  // Check Yard Pro status (Task 5.1)
...
async function runValidate() {
  ...
  try {
    ...
    const { createValidateCommand } = require('../.yard-core/cli/commands/validate/index.js');
    ...
    await validateCmd.parseAsync(['node', 'yard-core', ...validateArgs]);
  } catch (_error) {
...
async function runDoctor(options = {}) {
  const { runDoctorChecks } = require(path.join(__dirname, '..', '.yard-core', 'core', 'doctor'));
...
// Helper: Remove Yard sections from .gitignore
...
function showUninstallHelp() {
  console.log(`
Usage: npx yard-core uninstall [options]

Remove Yard from the current project.

Options:
  --force      Skip confirmation prompt
  --keep-data  Keep .yard/ directory (settings and history)
  --dry-run    Show what would be removed without removing
  -h, --help   Show this help message

What gets removed:
  - .yard-core/     Framework core files
  - docs/stories/   Story files (if created by Yard)
  - squads/         Squad definitions
  - .gitignore      Yard-added entries only

What is preserved (with --keep-data):
  - .yard/          Project settings and agent history
...
Examples:
  ...
  npx yard-core uninstall --force

  # See what would be removed
  npx yard-core uninstall --dry-run
...
function showDoctorHelp() {
  console.log(`
Usage: npx yard-core doctor [options]

Run health checks on your Yard installation.
...
Checks performed:
  • Required directories exist (.yard-core/, .yard/)
...
Examples:
  ...
  npx yard-core doctor --fix --dry-run
`);
}
...
async function runUninstall(options = {}) {
  ...
  // Items to remove
  const itemsToRemove = [
    { path: '.yard-core', description: 'Framework core' },
    { path: 'squads', description: 'Squad definitions' },
  ];

  // Optionally remove .yard
  if (!keepData) {
    itemsToRemove.push({ path: '.yard', description: 'Project data and settings' });
  }
  ...
  if (existingItems.length === 0) {
    console.log('ℹ️  No Yard installation found in this directory.');
    return;
  }
  ...
  if (!force) {
    ...
    const answer = await new Promise(resolve => {
      rl.question('⚠️  Are you sure you want to uninstall Yard? (y/N): ', resolve);
    });
    rl.close();
  ...
  if (gitignoreResult.removed && !quiet) {
    console.log(`  ✓ Cleaned ${gitignoreResult.lines} Yard entries from .gitignore`);
  }

  // Summary
  if (!quiet) {
    console.log('\n✅ Yard has been uninstalled.');
    if (keepData) {
      console.log('   Your project data in .yard/ has been preserved.');
    }
    console.log('\n   To reinstall: npx yard-core install');
  }
}
...
function showInstallHelp() {
  console.log(`
Usage: npx yard-core install [options]

Install Yard in the current directory.

Options:
  --force      Overwrite existing Yard installation
...
Smart Merge (Brownfield):
  ...
  Yard can merge new settings while preserving your customizations.

  - .env files: Adds new variables, preserves existing values
  - CLAUDE.md: Updates Yard sections, keeps your custom rules
...
Examples:
  ...
  npx yard-core install --force

  # Brownfield: merge configs automatically
  npx yard-core install --merge
...
function showInitHelp() {
  console.log(`
Usage: npx yard-core init <project-name> [options]

Create a new Yard project with the specified name.
...
Examples:
  ...
  npx yard-core init my-project --template minimal
  npx yard-core init my-project --force --skip-install
  npx yard-core init . --template enterprise
`);
}
...
async function initProject() {
  ...
  console.log(`Creating new Yard project: ${displayName}`);
  if (template !== 'default') {
