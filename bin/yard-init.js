...
/**
 * YARD-FullStack Installation Wizard v5 (LEGACY)
 ...
 *
 * Migration path:
 * - Use `npx yard-core` which routes through bin/yard.js to the new wizard
...
/**
 * Smart path resolution for Yard Core modules
 */
function resolveYardCoreModule(modulePath) {
  const yardCoreModule = path.join(__dirname, '..', '.yard-core', modulePath);

  const moduleExists =
    fs.existsSync(yardCoreModule + '.js') ||
    fs.existsSync(yardCoreModule + '/index.js') ||
    fs.existsSync(yardCoreModule);

  if (!moduleExists) {
    throw new Error(
      `Cannot find Yard Core module: ${modulePath}\n` +
        `Searched: ${yardCoreModule}\n` +
        'Please ensure yard-core is installed correctly.'
    );
  }

  return require(yardCoreModule);
}

// Load Yard Core modules
const { detectRepositoryContext } = resolveYardCoreModule(
  'infrastructure/scripts/repository-detector'
);
// PM adapters imported but not used directly (loaded dynamically)
// const { ClickUpAdapter } = resolveYardCoreModule('utils/pm-adapters/clickup-adapter');
// const { GitHubProjectsAdapter } = resolveYardCoreModule('utils/pm-adapters/github-adapter');
// const { JiraAdapter } = resolveYardCoreModule('utils/pm-adapters/jira-adapter');
...
async function main() {
  ...
  if (!context) {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      ...
      const defaultPackage = {
        ...
        version: '1.0.0',
        description: 'YARD-FullStack project',
        main: 'index.js',
  ...
  const installedManifestPath = path.join(projectRoot, '.yard-core', '.installed-manifest.yaml');
  const hasExistingInstall = fs.existsSync(installedManifestPath);

  if (hasExistingInstall && brownfieldUpgrader) {
    console.log(chalk.yellow('🔄 Existing YARD installation detected!'));
    console.log('');

    const sourceDir = path.join(context.frameworkLocation, '.yard-core');
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using YARD-FullStack?'),
      choices: [
        {
          name: '  Using YARD in a project ' + chalk.gray('(Framework files added to .gitignore)'),
          value: 'project-development',
        },
        {
          name:
            '  Developing YARD framework itself ' + chalk.gray('(Framework files are source code)'),
  ...
  const configPath = path.join(process.cwd(), '.yard', 'installation-config.yaml');
  // INS-2 Performance: Use async write instead of sync
  ...
  // Step 4b: Copy Yard Core files
  console.log('');

  const sourceCoreDir = path.join(context.frameworkLocation, '.yard-core');
  const targetCoreDir = path.join(context.projectRoot, '.yard-core');

  if (fs.existsSync(sourceCoreDir)) {
    // INS-2 Performance: Add spinner for file copy (AC9)
    const copySpinner = ora('Installing Yard Core files...').start();
    await fse.copy(sourceCoreDir, targetCoreDir);
    copySpinner.succeed(
      'Yard Core files installed ' +
    ...
    console.error(chalk.red('✗') + ' Yard Core files not found');
    process.exit(1);
  }
  ...
  if (!ides.includes('none')) {
    ...
    const ideRulesMap = {
      ...
      'github-copilot': { source: 'copilot-rules.md', target: '.github/chatmodes/yard-agent.md' },
    ...
    // Step 2: Install Yard Core agents and tasks for Claude Code
    ...
    if (ides.includes('claude')) {
      ...
      // Create YARD README for Claude Code
      const yardsReadme = path.join(
        context.projectRoot,
        '.claude',
        'commands',
        'YARD',
      ...
      await fse.ensureDir(path.dirname(yardsReadme));
      await fse.writeFile(
        yardsReadme,
        `# Yard Core Commands

This directory contains the core YARD-FullStack agents and tasks.
...
## Documentation
See .yard-core/user-guide.md for complete documentation.
`
      );
    ...
    // Step 3: Install Yard Core agents for Cursor
    ...
    if (ides.includes('cursor')) {
      ...
      // Create YARD README for Cursor
      const cursorReadme = path.join(context.projectRoot, '.cursor', 'rules', 'YARD', 'README.md');
      await fse.ensureDir(path.dirname(cursorReadme));
      await fse.writeFile(
        cursorReadme,
        `# Yard Core Rules

This directory contains the core YARD-FullStack agent rules for Cursor.
...
`
    ...
    // Step 4: Install Yard Core agents for other IDEs (Gemini, AntiGravity)
    // v4: Agents are in development/ module
    // INS-2 Performance: Uses cached agent files list
    ...
    if (ides.includes('github-copilot')) {
      ...
      if (cachedAgentFiles.length > 0) {
        ...
        for (const agentFile of cachedAgentFiles) {
          ...
          const targetPath = path.join(copilotModesDir, `yard-${agentName}.md`);
          await fse.copy(sourcePath, targetPath);
        }
  ...
  const possibleSquadsDirs = [
    ...
    path.join(context.projectRoot, 'node_modules', 'yard-core', 'squads'),
    path.join(context.projectRoot, 'node_modules', '@yard', 'fullstack', 'squads'),
  ];
  ...
  try {
    ...
    const validator = new PostInstallValidator(context.projectRoot, context.frameworkLocation, {
      ...
      // For production integrity checks, users should run `yard validate` which
    ...
    if (
      ...
      console.log(
        chalk.yellow('   Run ') +
          chalk.cyan('yard validate --repair') +
    ...
    if (process.env.DEBUG || process.env.YARD_DEBUG) {
      console.log(chalk.dim(`   Stack: ${validationError.stack}`));
    }
    console.log(chalk.dim('   Run `yard validate` to check installation integrity'));
  }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ YARD-FullStack installation complete! 🎉'));
  console.log('');
  ...
  console.log('  ' + chalk.dim('.yard-core/') + '           - Framework core files');

  if (ides.includes('claude')) {
    console.log('  ' + chalk.dim('.claude/'));
    console.log('    ' + chalk.dim('├─ CLAUDE.md') + '        - Main configuration');
    console.log('    ' + chalk.dim('└─ commands/'));
    console.log('      ' + chalk.dim('  ├─ YARD/') + '         - Core agents & tasks');
  ...
  if (ides.includes('cursor')) {
    ...
    console.log('      ' + chalk.dim('  ├─ YARD/') + '         - Core agent rules');
    if (selectedSquads && selectedSquads.length > 0) {
      selectedSquads.forEach((squad) => {
  ...
  if (ides.includes('claude')) {
    ...
    console.log('    • Browse: .claude/commands/YARD/agents/ for all available agents');
  }
  ...
  if (ides.includes('antigravity')) {
    ...
    console.log('    • Browse: .antigravity/rules/YARD/agents/ for all available agents');
  }

  console.log('  ' + chalk.yellow('General:'));
  console.log('    • Run ' + chalk.yellow('yard validate') + ' to verify installation integrity');
  console.log('    • Run ' + chalk.yellow('yard validate --repair') + ' to fix any missing files');
  console.log('    • Check .yard-core/user-guide.md for complete documentation');
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# YARD-FullStack Framework Files (auto-managed - do not edit)',
      '.yard-core/',
      'node_modules/.yard/',
      'outputs/minds/',
      '.yard-installation-config.yaml',
      '# End YARD-FullStack auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# YARD-FullStack Framework Files');
...
async function savePMConfig(pmTool, config, projectRoot) {
  ...
  const configPath = path.join(process.cwd(), '.yard', 'pm-config.yaml');
  // INS-2 Performance: Use async write
 ...
 * @param {string} sourceCoreDir - Path to installed .yard-core directory
 */
