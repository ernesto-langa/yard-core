...
/**
 * YARD-FullStack Minimal Installation
 *
...
console.log('\n⚠️  DEPRECATION WARNING: yard-minimal is deprecated.');
console.log('   The --minimal mode (squads) was replaced by Squads.');
console.log('   Running standard installation wizard instead.\n');

// Get the path to the main router (yard.js)
const routerPath = path.join(__dirname, 'yard.js');
...
child.on('error', (error) => {
  console.error('❌ Failed to start YARD:', error.message);
  process.exit(1);
});
