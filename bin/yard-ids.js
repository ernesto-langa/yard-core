...
/**
 * YARD IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   yard ids:query {intent}     — Query registry for matching artifacts
 *   yard ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   yard ids:health             — Run registry health check (self-healing)
 *   yard ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   yard ids:impact {entity-id} — Impact analysis for modifications
 *   yard ids:stats              — Registry statistics
 *   yard ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.yard-core', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.yard-core', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.yard-core', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.yard-core', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.yard-core', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
YARD IDS — Incremental Development System
...
Examples:
  yard ids:query "validate story drafts"
  yard ids:query "template rendering engine" --json
  yard ids:query "database migration" --type script
  yard ids:create-review
  yard ids:create-review --json
  yard ids:health
  yard ids:health --fix
  yard ids:check "validate yaml schema" --type task
  yard ids:impact create-doc
  yard ids:stats
  yard ids:stats --json
  yard ids:register .yard-core/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: yard ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: yard ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: yard ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: yard ids:register {file-path}');
    process.exit(1);
  }
