...
const ERROR_MESSAGES = {
  ...
  UNKNOWN_ERROR: {
    ...
    recovery: [
      'Check the installation log: .yard-install.log',
...
function formatErrorMessage(error, errorCode = null) {
  ...
  lines.push('');
  lines.push(chalk.gray('Installation log: .yard-install.log'));
  lines.push('');
...
function formatRollbackMessage(success, failedFiles = []) {
  ...
  if (success) {
    ...
    lines.push(chalk.cyan('  1. Check backup directory: .yard-backup/'));
    lines.push(chalk.cyan('  2. Manually restore failed files from backups'));
    lines.push(chalk.cyan('  3. Review installation log: .yard-install.log'));
  }
