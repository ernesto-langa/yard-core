#!/bin/bash
#
# YARD Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from yard-fullstack to yard-core (REPO 1)
...
#   --target-repo    Path to yard-core repository (default: ../yard-core)
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  YARD Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the yard-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone yard-core first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** yard-fullstack/docs/framework/
**Target:** yard-core/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from yard-fullstack to yard-core (REPO 1)

## Files Migrated

- coding-standards.md - Official YARD coding standards
...
- yard-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- yard-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from yard-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: yard-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to yard-core repository"
echo "    5. Update yard-fullstack deprecation warnings (Q3 2026)"
echo ""
