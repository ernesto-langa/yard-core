#!/bin/bash
# ============================================
# YARD PM Orchestration Script (pm.sh)
...
#   --output    - Custom output file path (default: /tmp/yard-output-{timestamp}.md)
#
# Environment Variables:
#   YARD_OUTPUT_DIR   - Directory for output files (default: /tmp)
#   YARD_DEBUG        - Enable debug logging (default: false)
#   YARD_TIMEOUT      - Timeout in seconds (default: 300)
...
# Configuration
OUTPUT_DIR="${YARD_OUTPUT_DIR:-/tmp}"
DEBUG="${YARD_DEBUG:-false}"
TIMEOUT="${YARD_TIMEOUT:-300}"
CLAUDE_CMD="${CLAUDE_CMD:-claude}"
INLINE_MODE="${YARD_INLINE_MODE:-false}"
...
show_help() {
  cat << EOF
YARD Multi-Modal Orchestration Script v${VERSION}
...
Environment Variables:
  YARD_OUTPUT_DIR    Output directory (default: /tmp)
  YARD_DEBUG         Enable debug mode (default: false)
  YARD_TIMEOUT       Timeout in seconds (default: 300)
  YARD_INLINE_MODE   Run without a visual terminal (default: false)
  CLAUDE_CMD         Claude CLI command (default: claude)
...
setup_paths() {
  ...
  if [[ -n "$CUSTOM_OUTPUT" ]]; then
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/yard-output-${timestamp}.md"
  fi

  LOCK_FILE="${OUTPUT_DIR}/yard-lock-${timestamp}.lock"
...
spawn_inline() {
  ...
  # Build the command
  local output=""
  output+="=== YARD Agent Session (Inline) ===$(printf '\n')"
...
spawn_terminal() {
  ...
  local full_cmd="echo '=== YARD Agent Session ===' && "
...
main() {
  ...
  log_info "YARD Multi-Modal Orchestration Script v${VERSION}"
