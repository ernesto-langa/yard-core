#!/bin/bash
# YARD Framework Update - v5.2 (Optimized)
#
# LOGIC:
...
#
# Usage: bash .yard-core/scripts/update-yard.sh

set -e

echo "⚡ YARD Update v5.2"
echo ""
...
# Validate: clean working tree for .yard-core
if [ -n "$(git status --porcelain .yard-core/)" ]; then
  echo "❌ Commit .yard-core changes first:"
  git status --short .yard-core/
  echo ""
  echo "Run: git add .yard-core && git commit -m 'your message'"
  exit 1
fi
...
git clone --depth 1 --filter=blob:none --sparse \
  https://github.com/ernesto-langa/yard-core.git \
...
git sparse-checkout set .yard-core
cd - > /dev/null

echo "✅ Fetched upstream"
echo ""

# Build file lists (relative paths without .yard-core/ prefix)
echo "📋 Scanning files..."
find .yard-core -type f | sed 's|^\.yard-core/||' | sort > "$TEMP_DIR/local-files.txt"
find "$TEMP_DIR/upstream/.yard-core" -type f | sed "s|^$TEMP_DIR/upstream/\.yard-core/||" | sort > "$TEMP_DIR/upstream-files.txt"

# Get git-tracked files (for DELETE detection) - ONE call instead of N calls
git ls-files .yard-core | sed 's|^\.yard-core/||' | sort > "$TEMP_DIR/tracked-files.txt"
...
while IFS= read -r rel_path; do
  if ! cmp -s ".yard-core/$rel_path" "$TEMP_DIR/upstream/.yard-core/$rel_path"; then
...
while IFS= read -r rel_path; do
  ...
  cp -a ".yard-core/$rel_path" "$TEMP_DIR/local-only/$rel_path"
...
if [ -s "$REPORT_DELETED" ]; then
  while IFS= read -r rel_path; do
    rm -f ".yard-core/$rel_path"
  done < "$REPORT_DELETED"
fi

# Copy all upstream files (creates new + overwrites existing)
rsync -a "$TEMP_DIR/upstream/.yard-core/" ".yard-core/"

# Restore local-only files
if [ -d "$TEMP_DIR/local-only" ] && [ "$(ls -A "$TEMP_DIR/local-only" 2>/dev/null)" ]; then
  rsync -a "$TEMP_DIR/local-only/" ".yard-core/"
fi

# Clean empty directories
find .yard-core -type d -empty -delete 2>/dev/null || true
...
echo ""
echo "Choose:"
echo "  ✅ Apply:  git add .yard-core && git commit -m 'chore: sync YARD framework'"
echo "  ❌ Cancel: git checkout -- .yard-core/"
echo ""
