...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.yard-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.yard-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.yard-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.yard-core/infrastructure');
