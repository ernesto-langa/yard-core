...
class JiraAdapter extends PMAdapter {
   ...
   * @param {string} config.project_key - Project key (e.g., "YARD")
   */
  constructor(config) {
    super(config);
  ...
  async pullStory(storyId) {
    try {
      ...
      // Map Jira status to YARD status
      const statusMapping = {
        'To Do': 'Draft',
  ...
  async updateStatus(storyId, status) {
    try {
      ...
      // Map YARD status to Jira transition
   ...
   * Get Jira issue
   * @private
   * @param {string} issueKey - Issue key (e.g., "YARD-123")
