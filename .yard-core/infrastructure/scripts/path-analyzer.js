...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .yard-core
  { pattern: /`\.yard-core\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.yard-core/development/tasks',
  template: '.yard-core/product/templates',
  checklist: '.yard-core/product/checklists',
  script: '.yard-core/infrastructure/scripts',
  data: '.yard-core/development/data',
  schema: '.yard-core/schemas',
  agent: '.yard-core/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.yard-core/development/agents', type: 'agent' },
    { dir: '.yard-core/development/tasks', type: 'task' },
    { dir: '.yard-core/product/templates', type: 'template' },
    { dir: '.yard-core/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.yard-core/')) {
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.yard-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .yard-core directory. Run from project root.');
    process.exit(1);
  }
