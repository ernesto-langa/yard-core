...
class ProjectStatusLoader {
  ...
  loadConfig() {
    try {
      const configPath = path.join(this.rootPath, '.yard-core', 'core-config.yaml');
  ...
  _resolveCacheFilePath() {
    try {
      ...
      if (normalizedGitDir !== normalizedCommonDir) {
        ...
        return path.join(this.rootPath, '.yard', `project-status-${worktreeHash}.yaml`);
      }
    } catch (error) {
      // Not a git repo or git not available - use default path
    }

    return path.join(this.rootPath, '.yard', 'project-status.yaml');
  }
 ...
 * @property {string} path - path.join(process.cwd(), '.yard', 'worktrees', story-id)
