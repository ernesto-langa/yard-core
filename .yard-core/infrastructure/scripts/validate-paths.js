...
function getDefaultOptions() {
  ...
  return {
    ...
    requiredFiles: [
      ...
      path.join(projectRoot, '.yard-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
    ],
    quiet: false,
    json: false,
  };
...
function validateSkillPathConventions(content, filePath) {
  const errors = [];
  if (!content.includes('.yard-core/development/agents/')) {
    errors.push(`${filePath} missing canonical source path ".yard-core/development/agents/"`);
  }
  if (!content.includes('.yard-core/development/scripts/generate-greeting.js')) {
