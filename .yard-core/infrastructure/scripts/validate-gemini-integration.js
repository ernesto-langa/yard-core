...
function validateGeminiIntegration(options = {}) {
  ...
  if (!commandFiles.includes('yard-menu.toml')) {
    errors.push(`Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, 'yard-menu.toml'))}`);
  }
  ...
  const requiredExtensionFiles = [
    'extension.json',
    'README.md',
    path.join('commands', 'yard-status.js'),
    path.join('commands', 'yard-agents.js'),
    path.join('commands', 'yard-validate.js'),
