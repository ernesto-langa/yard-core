...
const CORE_CONFIG_FILE = '.yard-core/core-config.yaml';
 ...
 * - .yard-core/core/** gets expanded one level deep
...
function expandProtectedPaths(protectedPaths, exceptions, projectRoot) {
  ...
  for (const globPath of protectedPaths) {
    if (globPath === '.yard-core/core/**') {
      const { dirs, files } = expandOneLevel('.yard-core/core', projectRoot);
