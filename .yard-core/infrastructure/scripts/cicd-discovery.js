...
class IntegrationSuggester {
  ...
  run: npx yard-core test --discover --coverage`,

      'gitlab-ci': `test:
  stage: test
  script:
    - npx yard-core test --discover --coverage
  coverage: '/Coverage: \\d+\\.\\d+%/'`,

      jenkins: `stage('Test') {
  steps {
    sh 'npx yard-core test --discover --coverage'
  }
}`,

      circleci: `- run:
    name: Run Tests
    command: npx yard-core test --discover --coverage`,
    };
  ...
  run: npx yard-core review --security-only
  continue-on-error: true`,

      'gitlab-ci': `security_scan:
  stage: test
  script:
    - npx yard-core review --security-only
  allow_failure: true`,

      jenkins: `stage('Security Scan') {
  steps {
    sh 'npx yard-core review --security-only'
  }
  post {
    failure {
      ...
      circleci: `- run:
    name: Security Scan
    command: npx yard-core review --security-only
    when: always`,
    };
  ...
  stage: .post
  script:
    - npx yard-core status --report-ci
  when: always`,

      jenkins: `post {
  always {
    sh 'npx yard-core status --report-ci'
  }
}`,
