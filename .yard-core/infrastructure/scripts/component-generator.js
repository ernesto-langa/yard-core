...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'yard-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'yard-core', 'agents'),
      task: path.join(this.rootPath, 'yard-core', 'tasks'),
      workflow: path.join(this.rootPath, 'yard-core', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using YARD template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'YARD';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'yard-core', 'tasks', `${taskId}.md`);
