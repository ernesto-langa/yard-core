...
const GitignoreTemplates = {
  YARD_BASE: 'gitignore-yard-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.YARD_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by YARD Documentation Integrity System');
...
/**
 * Merges YARD ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if YARD section already exists
  if (existingContent.includes('YARD Integration Section')) {
    console.log('YARD section already exists in .gitignore, skipping merge');
...
# YARD Local Configuration
.yard-core/local/
.yard-core/*.local.yaml
.yard-core/logs/
.yard-core/cache/
...
/**
 * Checks if a .gitignore file has YARD integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if YARD section exists
 */
function hasYardIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('YARD Integration Section') || content.includes('.yard-core/');
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('YARD Integration Section')) {
      if (yardStart === -1) {
        yardStart = i;
      }
    }

    if (line.includes('End of YARD Integration Section')) {
      yardEnd = i;
    }

    if (line.startsWith('#') && !line.includes('YARD')) {
  ...
  if (yardStart !== -1) {
    sections.yardSection = {
      start: yardStart,
      end: yardEnd !== -1 ? yardEnd : lines.length - 1,
    };
  }
