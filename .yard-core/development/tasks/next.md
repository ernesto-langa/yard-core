...
pre-conditions:
  ...
  - [ ] Session state exists (optional)
    tipo: pre-condition
    blocker: false
    validação: Check .yard/session-state.json exists
...
```javascript
const SuggestionEngine = require('.yard-core/workflow-intelligence/engine/suggestion-engine');
...
```javascript
const formatter = require('.yard-core/workflow-intelligence/engine/output-formatter');
