...
- campo: gotchas_file
  tipo: string
  destino: .yard/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .yard/gotchas.json
  persistido: true
...
pre-conditions:
  - [ ] Project has .yard directory initialized
    ...
    blocker: false
    validação: |
      Check if .yard/ directory exists, create if not
    error_message: "Creating .yard/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .yard/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .yard/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.yard/gotchas.md` and `.yard/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.yard-core/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.yard/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.yard/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.yard-core/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.yard-core/infrastructure/scripts/gotchas-documenter');
...
node .yard-core/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .yard-core/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .yard-core/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .yard-core/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .yard-core/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .yard-core/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .yard-core/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.yard-core/development/tasks/capture-session-insights.md` - Provides input insights
- `.yard-core/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.yard-core/development/tasks/spec-write-spec.md` - May reference gotchas
