...
- **Tool:** code-analyzer
  ...
  - **Source:** .yard-core/utils/code-analyzer.js
...
- **Script:** analyze-codebase.js
  ...
  - **Language:** JavaScript
  - **Location:** .yard-core/scripts/analyze-codebase.js
...
Systematically audit all utilities in `.yard-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .yard-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .yard-core/agents .yard-core/tasks Squads 2>/dev/null | wc -l)
