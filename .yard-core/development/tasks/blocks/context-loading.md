...
Load YARD project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .yard-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .yard-core/core-config.yaml
...
```javascript
const { loadContext } = require('.yard-core/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.yard/gotchas.json` | Known issues and workarounds |
| `.yard-core/data/technical-preferences.md` | User-defined patterns |
| `.yard-core/core-config.yaml` | Project configuration |
