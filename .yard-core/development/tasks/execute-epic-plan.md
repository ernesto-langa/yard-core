...
- campo: epic_state
  tipo: object
  destino: File system (.yard/epic-{epicId}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Template workflow must exist
    ...
    blocker: true
    validação: |
      execution.template must resolve to .yard-core/development/workflows/{template}.yaml
  ...
  - [ ] For action=continue, state file must exist
    ...
    blocker: true
    validação: |
      .yard/epic-{epicId}-state.yaml must exist with status=active
...
post-conditions:
  - [ ] State file updated with current wave progress
    ...
    blocker: true
    validação: |
      .yard/epic-{epicId}-state.yaml exists and reflects completed waves/stories
...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .yard/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.yard-core/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.yard-core/development/workflows/`
...
**1. Load state** from `.yard/epic-{epicId}-state.yaml`
...
State preserved at: .yard/epic-{epicId}-state.yaml
