...
autoClaude:
  ...
  inputs:
    ...
    - name: projectContext
      type: file
      path.join(process.cwd(), '.yard', project-context.yaml
      required: false
...
step_11:
  ...
  actions:
    ...
    - action: update_dashboard_status
      script: |
        const { PlanTracker } = require('.yard-core/infrastructure/scripts/plan-tracker.js');
...
recovery:
  ...
  attempt_tracking:
    storage: .yard/recovery/{storyId}/{subtaskId}.json
    schema:
      subtaskId: string
      attempts: array
