...
const configPath = path.join(__dirname, '../../.yard-core/core-config.yaml');
...
mkdir -p .backups
cp -r .yard-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .yard-core/agents .yard-core/tasks .yard-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .yard-core/agents/ .yard-core/tasks/ .yard-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .yard-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .yard-core/utils-archive
```

**3.2 Create Archive README**

Create `.yard-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .yard-core/utils-archive/utility-name.js .yard-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .yard-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .yard-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .yard-core/utils
```

**Selective Restoration**:
```bash
cp .yard-core/utils-archive/specific-utility.js .yard-core/scripts/
```
...
- **Tool:** task-runner
  ...
  - **Source:** .yard-core/core/task-runner.js

- **Tool:** logger
  - **Purpose:** Execution logging and error tracking
  - **Source:** .yard-core/utils/logger.js
...
- **Script:** execute-task.js
  ...
  - **Language:** JavaScript
  - **Location:** .yard-core/scripts/execute-task.js
...
Update utility count in `.yard-core/core-config.yaml`:
...
for util in $(ls .yard-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .yard-core/agents .yard-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .yard-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .yard-core/utils
git checkout .yard-core/core-config.yaml
```
...
1. `.yard-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.yard-core/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.yard-core/core-config.yaml` - Corrected utility count
