...
- campo: devops_setup_report
  tipo: object
  destino: File system (.yard/devops-setup-report.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Not already configured (idempotency check)
    ...
    blocker: false
    validação: |
      Check .yard/devops-setup-report.yaml existence
...
post-conditions:
  ...
  - [ ] DevOps setup report generated
    ...
    blocker: false
    validação: |
      Test-Path ".yard/devops-setup-report.yaml"
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validação: |
      .yard/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".yard/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".yard-core/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".yard-core/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".yard-core/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".yard-core/infrastructure/templates/coderabbit.yaml.template" -Raw
...
Write-Host "✅ Setup report saved to .yard/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .yard/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .yard/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝
