...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .yard-core/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
const { RegistryLoader } = require(path.resolve('.yard-core/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.yard-core/core/ids/incremental-decision-engine'));
