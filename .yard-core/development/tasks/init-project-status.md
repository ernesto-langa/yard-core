...
- **Tool:** project-scaffolder
  ...
  - **Source:** .yard-core/scripts/project-scaffolder.js

- **Tool:** config-manager
  - **Purpose:** Initialize configuration files
  - **Source:** .yard-core/utils/config-manager.js
...
**Action:** Read `.yard-core/core-config.yaml` and check `projectStatus.enabled`

**Logic:**
```javascript
const config = yaml.load(fs.readFileSync('.yard-core/core-config.yaml'));
...
projectStatus:
  ...
  components:
    ...
    currentEpic: true
    currentStory: true
  statusFile: .yard/project-status.yaml
  maxModifiedFiles: 5
  maxRecentCommits: 2
```
...
```javascript
const { loadProjectStatus } = require('./.yard-core/scripts/project-status-loader.js');
const status = await loadProjectStatus();
```

**Verification:** Check that `.yard/project-status.yaml` was created with valid content.
...
**Confirmation:**
```
✅ Initialized project status cache (.yard/project-status.yaml)
```
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.yard-core/scripts/project-status-loader.js');
const status = await loadProjectStatus();
...
**Action:** Ensure `.yard/project-status.yaml` is gitignored

**Check:** Look for `.yard/project-status.yaml` entry in `.gitignore`
...
**Confirmation:**
```
✅ Added .yard/project-status.yaml to .gitignore
```
...
- `.yard/project-status.yaml` - Status cache file (gitignored)

### Files Modified

- `.yard-core/core-config.yaml` - projectStatus section enabled (if was disabled)
...
- [ ] `.yard/project-status.yaml` exists and contains valid YAML
- [ ] `core-config.yaml` has `projectStatus.enabled: true`
- [ ] `.gitignore` includes `.yard/project-status.yaml`
...
**Error:**
```
❌ Could not find .yard-core/core-config.yaml
...
- `.yard-core/scripts/project-status-loader.js` - Core status loader
...
- **Documentation:** `docs/guides/project-status-feature.md`
- **Config:** `.yard-core/core-config.yaml` (projectStatus section)
