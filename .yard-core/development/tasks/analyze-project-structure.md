...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .yard-core/
...
```yaml
pre-conditions:
  - [ ] Project has .yard-core/ directory
    type: pre-condition
    blocker: true
    validation: |
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .yard-core/ configuration analyzed
...
const scanLocations = {
  ...
  tasks: '.yard-core/development/tasks/',
  data: '.yard-core/data/'
};
```
...
```
.yard-core/infrastructure/services/{service-name}/
...
```
.yard-core/infrastructure/services/{service_name}/
├── README.md
├── index.ts
...
const { getDependencyGraph, getComplexityAnalysis } = require('.yard-core/core/code-intel/helpers/planning-helper');
