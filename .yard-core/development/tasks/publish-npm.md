...
5. PUBLISH
   ...
   - Verify: npm view yard-core@preview version === {new_version}

6. SMOKE TEST
   - Create temp directory
   - npm init -y
   - npm install yard-core@preview
...
8. REPORT
   ...
   - "Test with: npm install yard-core@preview"
...
- [ ] `npm view yard-core@preview` returns new version
...
1. VERIFY PREVIEW EXISTS
   - npm view yard-core@preview version
   - If no preview: HALT with "No preview version found"

2. CONFIRM WITH USER
   - Display: "Promote v{X.Y.Z} from preview to latest?"
   - Show current latest: npm view yard-core@latest version
   - Require explicit confirmation

3. PROMOTE
   - npm dist-tag add yard-core@{version} latest

4. VERIFY
   - npm view yard-core@latest version === {version}
...
6. REPORT
   ...
   - Release URL
   - "Install with: npm install yard-core"
```

#### Pre-conditions

- [ ] Preview version exists (`npm view yard-core@preview`)
...
- [ ] `npm view yard-core@latest` returns promoted version
...
2. INSTALL
   - npm install yard-core@{tag} (default: latest)
   - Record: install time, exit code, warnings

3. VERIFY FILES
   - Check node_modules/yard-core/ contains:
     - .yard-core/core-config.yaml
     - .yard-core/constitution.md
     - .yard-core/development/agents/ (non-empty)
     - .yard-core/development/tasks/ (non-empty)
     - .claude/hooks/synapse-engine.cjs
...
4. TEST INSTALLER (optional, if --full flag)
   - npx yard-core install --preset minimal
   - Verify: .yard-core/ created
...
- `--full`: Also test `npx yard-core install`
...
npm view yard-core versions --json

# 2. Point latest back to previous version
npm dist-tag add yard-core@{previous-good-version} latest

# 3. Deprecate broken version with message
npm deprecate yard-core@{broken-version} "Known issues, use v{previous-good-version}"

# 4. Verify
npm view yard-core@latest version
```

**Important:** `npm deprecate` shows a warning on install but does NOT prevent installation.
To fully block a version, use `npm unpublish yard-core@{version}` (within 72h of publish only).
