...
- campo: workflow_state
  tipo: object
  destino: File system (.yard/{instance-id}-engine-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active engine state file must exist
    ...
    blocker: true
    validação: |
      Check .yard/{instance-id}-engine-state.yaml exists with status=active
...
post-conditions:
  ...
  - [ ] State file created with all step outputs
    ...
    blocker: true
    validação: |
      Verify .yard/{instance-id}-engine-state.yaml exists and contains outputs
...
- **Tool:** workflow-state-manager
  ...
  - **Source:** .yard-core/development/scripts/workflow-state-manager.js

- **Tool:** workflow-validator
  - **Purpose:** Validate workflow before starting
  - **Source:** .yard-core/development/scripts/workflow-validator.js
...
- Template: `subagent-step-prompt.md` available at `.yard-core/development/templates/`
...
- `core` → `.yard-core/development/workflows/{workflow_name}.yaml`
...
State preserved at: .yard/{instance_id}-engine-state.yaml
```
...
State saved to: .yard/{instance_id}-engine-state.yaml
```
...
1. **Load template** from `.yard-core/development/templates/subagent-step-prompt.md`
...
resolve_agent_path(agent_ref, target_context, squad_name):
  ...
  IF target_context == "hybrid":
    ...
    core_path = ".yard-core/development/agents/{agent_ref}.md"
...
resolve_task_path(uses_ref, target_context, squad_name):
  IF target_context == "core":
    RETURN ".yard-core/development/tasks/{uses_ref}.md"
  IF target_context == "squad":
    RETURN "squads/{squad_name}/tasks/{uses_ref}.md"
  IF target_context == "hybrid":
    squad_path = "squads/{squad_name}/tasks/{uses_ref}.md"
    core_path = ".yard-core/development/tasks/{uses_ref}.md"
    IF squad_path exists → RETURN squad_path
    IF core_path exists → RETURN core_path
...
```yaml
# .yard/{instance-id}-engine-state.yaml
engine_state:
  workflow_id: {id}
  workflow_name: {name}
