...
} = require('./.yard-core/scripts/decision-recorder');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.yard-core/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.yard-core/core/code-intel/helpers/dev-helper`
...
7. Execute `.yard-core/product/checklists/story-dod-checklist.md`
...
- `.yard-core/product/checklists/story-dod-checklist.md` - Definition of Done checklist
