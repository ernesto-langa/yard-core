...
Complete environment bootstrap for new YARD projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
- campo: environment_report
  tipo: object
  destino: File system (.yard/environment-report.yaml)
  persistido: true
...
post-conditions:
  ...
  - [ ] Environment report generated
    ...
    blocker: false
    validação: |
      Check .yard/environment-report.yaml exists
...
acceptance-criteria:
  ...
  - [ ] Project structure follows YARD conventions
    tipo: acceptance-criterion
    blocker: false
    validação: |
      Check docs/, .yard/, and package.json exist
...
- **Tool:** cli-checker
  ...
  - **Source:** .yard-core/infrastructure/scripts/cli-checker.js

- **Tool:** github-cli
  - **Purpose:** Repository creation and authentication
  - **Source:** .yard-core/infrastructure/tools/cli/github-cli.yaml
...
- Cache detection results in .yard/environment-report.yaml
...
---
*Generated by YARD Environment Bootstrap*
...
- Initialize YARD-Framework project structure
...
**Action:** Create YARD-compliant project structure
...
# Create .yard/config.yaml
@"
# YARD Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".yard/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with YARD-Framework",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.yard/user-config.yaml`
...
```bash
# Create ~/.yard/ with secure permissions
mkdir -p ~/.yard
chmod 700 ~/.yard

# Write user-config.yaml with selected profile
cat > ~/.yard/user-config.yaml << EOF
# YARD User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('.yard-core/core/config/config-resolver');
...
- `~/.yard/` directory must have permissions 700
- `~/.yard/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".yard/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .yard/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ YARD ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
║                                                                            ║
║  Full workflow: .yard-core/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .yard/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • YARD help: @yard-master *help                                           ║
...
- [YARD Greenfield Workflow](.yard-core/development/workflows/greenfield-fullstack.yaml)
