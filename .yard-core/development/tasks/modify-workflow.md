...
- Target workflow must exist (path resolved from target_context):
  - `core` → `.yard-core/development/workflows/`
...
- Resolve workflow path based on target_context:
  - `core` → `.yard-core/development/workflows/{workflow-name}.yaml`
...
- Create timestamped backup in same context:
  - `core` → `.yard-core/development/workflows/.backups/{workflow-name}.yaml.{timestamp}`
