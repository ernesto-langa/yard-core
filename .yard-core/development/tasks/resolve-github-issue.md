...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.yard-core/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.yard-core/development/agents/)
...
steps:
  ...
  3_handle_edge_cases:
    ...
    known_pitfalls:
      - Email addresses inside strings may match rename patterns (Issue #159: security@synkra/yard-core.dev)
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .yard-core/install-manifest.yaml
    when: Any file in .yard-core/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .yard-core/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .yard-core/development/agents/ or ide-sync/
```
...
- **Pitfall:** `@synkra/yard-core` inside email `security@synkra/yard-core.dev` was corrupted
