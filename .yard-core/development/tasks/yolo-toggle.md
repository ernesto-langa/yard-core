...
Read the current permission mode from `.yard/config.yaml` under `permissions.mode`.
...
Update `.yard/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.yard/` directory if needed.
...
```javascript
const { cycleMode } = require('./.yard-core/core/permissions');
...
- If `.yard/config.yaml` cannot be read, start from default `ask` mode
...
version: 1.0.0
dependencies:
  - .yard-core/core/permissions/permission-mode.js
  - .yard-core/core/permissions/index.js
tags:
  - permissions
  - mode-toggle
  - universal-command
