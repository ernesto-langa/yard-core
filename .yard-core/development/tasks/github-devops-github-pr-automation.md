...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.yard-core', 'core-config.yaml');
...
```javascript
const { generateImpactSummary } = require('.yard-core/core/code-intel/helpers/devops-helper');
...
### NPM Package with Semantic-Release (yard-core)
```yaml
github:
  pr:
