...
outputs:
  - name: service_directory
    type: directory
    location: ".yard-core/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .yard-core/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .yard-core/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for YARD agents
...
const { checkBeforeWriting } = require('.yard-core/core/code-intel/helpers/dev-helper');
...
// Check uniqueness
const targetDir = `.yard-core/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.yard-core/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .yard-core/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .yard-core/infrastructure/services/{service_name}
