...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"yard-core/agents/improved-agent.md"`
...
- **pr_url**: `string`
  ...
  - **Example**: `"https://github.com/ernesto-langa/yard-core/pull/123"`
...
2. **Validate Repository State**
   - Check if `yard-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `yard-core/agents/*` → "agent"
     - `yard-core/tasks/*` → "task"
     - `yard-core/tools/*` → "tool"
...
- [ ] Fork of yard-core exists
  - **Validation**: `gh repo view {user}/yard-core` succeeds
