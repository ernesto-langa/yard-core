...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .yard-core exists
...
```javascript
const PatternExtractor = require('.yard-core/infrastructure/scripts/pattern-extractor');
...
Examples:
  ...
  *extract-patterns save                      # Save to .yard/patterns.md
...
Output File:
  Default location: .yard/patterns.md
...
Patterns saved to: .yard/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .yard/patterns.md exists and is valid markdown
...
references:
  - path.join(process.cwd(), '.yard', patterns.md
...
dependencies:
  modules:
    - .yard-core/infrastructure/scripts/pattern-extractor.js
  tasks: []
  referenced_by:
    - spec-write.md
tags:
