...
*learn-patterns --component yard-core/agents/developer.md --analyze
...
class LearnPatternsTask {
  ...
  async loadModification(modificationId) {
    ...
    const sources = [
      path.join(this.rootPath, '.yard', 'modifications', `${modificationId}.json`),
      path.join(this.rootPath, '.yard', 'history', `${modificationId}.json`),
      path.join(this.rootPath, '.yard', 'proposals', `${modificationId}.json`)
    ];
