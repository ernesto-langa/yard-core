...
IDE-FILE-RESOLUTION:
  ...
  - Example: create-doc.md → .yard-core/development/tasks/create-doc.md
...
activation-instructions:
  ...
  - STEP 2.5: |
      ...
      Check user_profile using config-resolver's resolveConfig():
        ...
        - If user_profile === 'bob':
          → Load bob-orchestrator.js module from .yard-core/core/orchestration/bob-orchestrator.js
      ...
      Module: .yard-core/core/config/config-resolver.js
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .yard-core/development/scripts/unified-activation-pipeline.js pm
  - STEP 3.5: |
      ...
      1. Run data lifecycle cleanup first:
         - const { runStartupCleanup } = require('.yard-core/core/orchestration/data-lifecycle-manager')
         - await runStartupCleanup(projectRoot) // Cleanup locks, sessions >30d, snapshots >90d

      2. Check for existing session state:
         - const { BobOrchestrator } = require('.yard-core/core/orchestration/bob-orchestrator')
      ...
      Module: .yard-core/core/orchestration/bob-orchestrator.js (Story 12.5)
      Module: .yard-core/core/orchestration/data-lifecycle-manager.js (Story 12.5)
      Task: .yard-core/development/tasks/session-resume.md
...
persona:
  ...
  orchestration_constraints:
    ...
    integration:
      module: .yard-core/core/orchestration/terminal-spawner.js
      script: .yard-core/scripts/pm.sh
      executor_assignment: .yard-core/core/orchestration/executor-assignment.js
...
2. PRD templates in `.yard-core/product/templates/`
