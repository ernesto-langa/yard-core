...
O Yard oferece instalação simplificada através de um instalador interativo completo.
...
Se você está contribuindo para o Yard em si:

```bash
# Clone o repositório
git clone https://github.com/ernesto-langa/yard-core
cd yard-core
...
```
your-project/
├── .yard-core/                    # ✅ Framework core instalado
...
npx yard-core@latest install --force-upgrade

# OU interativo (todos os RCs)
npx yard-core@latest install
# → Menu aparece:
#   1. Keep current version (v1.0.0-rc.8)
#   2. Upgrade Yard core (v1.0.0-rc.8 → v1.0.0-rc.10)
...
# Ajuda do instalador
npx yard-core@latest install --help
```
...
- `.yard-core/` é código fonte (commitado)
...
# Cria .yard-installation-config.yaml
...
npx yard-core@1.0.0-rc.10 install --force-upgrade
```
...
O Yard impõe integridade de repositório através de um agente centralizado de DevOps.
...
O Yard oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
Adicione templates próprios em `yard-core/templates/`:
