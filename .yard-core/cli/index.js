 ...
 *
 * Main entry point for the YARD CLI with Commander.js integration.
...
function createProgram() {
  ...
  program
    ...
    .description('YARD-FullStack: AI-Orchestrated System for Full Stack Development')
    .addHelpText('after', `
Commands:
  ...
  pro               YARD Pro license management (activate, status, deactivate, features)
  ...
  install           Install YARD in current project
  init <name>       Create new YARD project
...
Examples:
  $ yard workers search "json transformation"
  $ yard workers list --category=data
  $ yard manifest validate
  $ yard manifest regenerate
  $ yard qa run
  $ yard qa status
  ...
  $ yard mcp status
  $ yard metrics show
  $ yard metrics record --layer 1 --passed
  $ yard metrics seed --days 30
  $ yard migrate --dry-run
  $ yard migrate --from=2.0 --to=2.1
  $ yard generate pmdr --title "Feature X Decision"
  $ yard generate adr --save
  $ yard generate list
  $ yard config show
  ...
  $ yard pro activate --key PRO-XXXX-XXXX-XXXX-XXXX
  $ yard pro status
  $ yard pro deactivate
  $ yard pro features
  $ yard pro validate
