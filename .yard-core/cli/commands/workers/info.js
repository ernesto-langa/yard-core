 ...
 *
 * Implements `yard workers info <id>` CLI command.
...
function createInfoCommand() {
  ...
  info
    ...
    .addHelpText('after', `
Examples:
  $ yard workers info json-csv-transformer
  $ yard workers info architect-checklist --format=json
  $ yard workers info create-story --format=yaml
  $ yard workers info data-analyzer --verbose

Output Formats:
  pretty   Formatted text with sections and boxes (default)
  json     JSON object with all metadata
  yaml     YARD document with all metadata
`)
    .action(executeInfo);
