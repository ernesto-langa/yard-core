 ...
 */
async function validateStructure(yardCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  const rootEntries = await fs.promises.readdir(yardCoreDir, { withFileTypes: true });
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(yardCoreDir);
  ...
  result.imports = await verifyImports(yardCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push('  3. Report issues: github.com/yard/issues');
  lines.push('');
  lines.push('To rollback: yard migrate --rollback');
