...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(yardCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .yard-core directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(yardCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(yardCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect YARD version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .yard-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(yardCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(yardCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(yardCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(yardCoreDir, moduleName);
