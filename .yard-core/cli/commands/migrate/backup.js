...
/**
 * Create backup of .yard-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  if (!fs.existsSync(yardCoreDir)) {
    throw new Error('No .yard-core directory found. Is this a YARD v2.0 project?');
  }
  ...
  const files = await getAllFiles(yardCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(yardCoreDir, srcFile);
    const destFile = path.join(backupDir, '.yard-core', relativePath);
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid YARD backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.yard-core', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.yard-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.yard-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.yard-backup-')) {
