 ...
 *   - .yard-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.yard/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.yard/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.yard-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.yard-core', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.yard-core', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.yard-core', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.yard-core', 'core-config.backup.yaml');
 ...
 *
 * @returns {string} Path to ~/.yard/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.yard');
...
/**
 * Write user config to ~/.yard/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.yard', 'user-config.yaml');
...
/**
 * Write project config to .yard-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.yard-core', 'project-config.yaml');
