 ...
 */
const CONFIG_FILES = {
  framework: '.yard-core/framework-config.yaml',
  project: '.yard-core/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.yard-core/local-config.yaml',
  legacy: '.yard-core/core-config.yaml',
  user: path.join(os.homedir(), '.yard', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.yard/user-config.yaml)
...
/**
 * Ensure the ~/.yard/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.yard/user-config.yaml).
