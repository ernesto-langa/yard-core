...
```javascript
const { updateSessionAfterCommand } = require('./.yard-core/scripts/command-execution-hook');
...
- **Path:** `.yard-core/.session/current-session.json`
...
```bash
# Clear session
rm .yard-core/.session/current-session.json

# Test new session greeting
node .yard-core/development/scripts/test-greeting-system.js

# Simulate command
node -e "require('./.yard-core/scripts/command-execution-hook').updateSessionAfterCommand('dev', 'develop-yolo')"
...
```bash
ls -la .yard-core/.session/
cat .yard-core/.session/current-session.json
```
