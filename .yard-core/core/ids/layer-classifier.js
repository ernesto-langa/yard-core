 ...
 *
 * Layer Model:
 *   L1 (Framework Core)      — .yard-core/core/, bin/, constitution.md
 *   L2 (Framework Templates)  — .yard-core/development/, infrastructure/, product/
 *   L3 (Project Config)       — .yard-core/data/, MEMORY.md, .claude/, *-config.yaml
...
const LAYER_RULES = [
  // --- L1: Framework Core ---
  { layer: 'L1', test: (p) => p.startsWith('.yard-core/core/') },
  { layer: 'L1', test: (p) => p.startsWith('bin/') },
  { layer: 'L1', test: (p) => p === '.yard-core/constitution.md' },

  // --- L3: Project Config (before L2 to catch MEMORY.md inside agents/) ---
  { layer: 'L3', test: (p) => p.startsWith('.yard-core/data/') },
  ...
  { layer: 'L2', test: (p) => p.startsWith('.yard-core/development/') },
  { layer: 'L2', test: (p) => p.startsWith('.yard-core/infrastructure/') },
  { layer: 'L2', test: (p) => p.startsWith('.yard-core/product/') },
