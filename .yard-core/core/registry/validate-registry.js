...
async function validateFields(registryPath) {
  ...
  for (const worker of registry.workers) {
    ...
    // Check path format
    if (worker.path && !worker.path.startsWith('.yard-core/')) {
      issues.push({ id: worker.id, issue: 'Invalid path prefix (must start with .yard-core/)' });
    }
  }
...
async function main() {
  ...
  const registryPath = path.join(baseDir, '.yard-core/core/registry/service-registry.json');
  const schemaPath = path.join(baseDir, '.yard-core/core/registry/registry-schema.json');
