 ...
 *
 * @module yard-core/doctor/checks/code-intel
...
async function run(context) {
  const codeIntelDir = path.join(context.projectRoot, '.yard-core', 'core', 'code-intel');
  ...
  try {
    ...
    if (!available) {
      ...
      const registryPath = path.join(context.projectRoot, '.yard-core', 'data', 'entity-registry.yaml');
      if (fs.existsSync(registryPath)) {
        ...
        return {
          ...
          fixCommand: 'node .yard-core/development/scripts/populate-entity-registry.js',
        };
      }
