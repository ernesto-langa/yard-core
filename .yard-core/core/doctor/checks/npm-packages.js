 ...
 * 2. (INS-4.12) .yard-core/node_modules/ exists and contains all declared deps
 *
 * @module yard-core/doctor/checks/npm-packages
  ...
  if (fs.existsSync(aioxCorePackageJson)) {
    if (!fs.existsSync(aioxCoreNodeModules)) {
      return {
        check: name,
        status: 'FAIL',
        message: 'node_modules present, but .yard-core/node_modules/ missing',
        fixCommand: 'cd .yard-core && npm install --production',
      };
    }
    ...
    try {
      ...
      if (missing.length > 0) {
        return {
          check: name,
          status: 'FAIL',
          message: `node_modules present, but .yard-core missing deps: ${missing.join(', ')}`,
          fixCommand: 'cd .yard-core && npm install --production',
        };
      }
    } catch {
