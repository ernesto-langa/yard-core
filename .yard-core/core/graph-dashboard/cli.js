...
/**
 * Write HTML graph to .yard/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), '.yard');
 ...
 * Writes to .yard/graph.dot, .yard/graph.mmd, or .yard/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), '.yard-core/data/entity-registry.yaml');
