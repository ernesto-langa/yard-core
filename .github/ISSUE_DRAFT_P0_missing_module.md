# [P0] Missing Yard Core module: utils/repository-detector - Blocks Linux Installation
...
- Installer should locate Yard Core module successfully
...
2. **Missing File/Directory**
   - `.yard-core/utils/repository-detector.js` may not exist in repository
...
2. **Fix path resolution in `bin/yard-init.js:43`:**
   ...
   ```javascript
   // Before (suspected):
   const detector = require('./.yard-core/utils/repository-detector');

   // After (cross-platform):
   const path = require('path');
   const detectorPath = path.resolve(__dirname, '..', '.yard-core', 'utils', 'repository-detector');
...
3. **Add file existence check:**
   ...
   if (!fs.existsSync(detectorPath + '.js')) {
     console.error('Yard Core module not found.');
