...
All notable changes to YARD-Framework will be documented in this file.
...
- Pro wizard (`npx yard-core install`) now auto-installs `@ernesto-langa/yard-pro` package during Step 2, fixing "Pro package not found" error in greenfield and brownfield projects.
...
- **Rebranding to YARD** (OSR-9)
  - Brand investigation complete
  - Namespace updated to YardAI
...
- **Scripts Path Consolidation** (6.16)
  - Standardized script locations under `.yard-core/scripts/`
...
- **Scripts Location** - Consolidated under `.yard-core/scripts/` for consistency
...
- **Private squads** - Moved to separate private repository (`yard-squads`)
  ...
  - Repository: https://github.com/YardAI/yard-squads (PRIVATE)
- **Internal development tools** - Moved to separate private repository (`yard-dev-tools`)
  ...
  - Repository: https://github.com/YardAI/yard-dev-tools (PRIVATE)
- **hybrid-ops squad** - Moved to separate repository for independent maintenance
  ...
  - Repository: https://github.com/YardAI/yard-hybrid-ops-pedro-valerio
...
- Breaking Change: hybrid-ops no longer bundled with yard-core
...
- Prevents confusion when running `npx yard-core install` from home directory

### Changed
- Early exit with `process.exit(1)` when NPX temporary context detected
- Help message provides actionable solution: `cd /path/to/your/project && npx yard-core install`
