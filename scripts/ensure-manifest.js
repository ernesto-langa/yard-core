...
function shouldCheckManifest(stagedFiles) {
  ...
  return stagedFiles.some((file) => {
    if (file === '.yard-core/install-manifest.yaml') return false;
    return file.startsWith('.yard-core/');
  });
}
...
async function main() {
  ...
  if (!shouldCheckManifest(stagedFiles)) {
    console.log('ℹ️ manifest: no .yard-core changes staged, skipping check');
    return;
  }
  ...
  execSync('git add .yard-core/install-manifest.yaml', { stdio: 'inherit' });
