...
/**
 * Dependency Graph Validator — .yard-core/package.json completeness
 * Story INS-4.12 (AC1, AC4, AC6)
 *
 * Scans all .js files in .yard-core/development/scripts/ for require() calls,
 * then verifies each non-builtin, non-relative package is declared in
 * .yard-core/package.json dependencies.
 *
 * Exit codes: 0 = PASS, 1 = FAIL (missing deps found)
 * Usage: node scripts/validate-yard-core-deps.js
 */
...
const YARD_CORE_DIR = path.join(PROJECT_ROOT, '.yard-core');
const SCRIPTS_DIR = path.join(YARD_CORE_DIR, 'development', 'scripts');
const PACKAGE_JSON = path.join(YARD_CORE_DIR, 'package.json');
...
function main() {
  console.log('--- .yard-core Dependency Validation (INS-4.12) ---\n');

  // Read .yard-core/package.json
  if (!fs.existsSync(PACKAGE_JSON)) {
    console.error('FAIL: .yard-core/package.json not found');
    process.exit(1);
  }
  ...
  if (scripts.length === 0) {
    console.log('WARN: No .js files found in .yard-core/development/scripts/');
    process.exit(0);
  }

  console.log(`Scanning ${scripts.length} scripts in .yard-core/development/scripts/\n`);
  ...
  console.log(`Declared in .yard-core/package.json: ${declared.size}`);
  ...
  if (errors.length > 0) {
    ...
    console.error('\nFix: Add missing packages to .yard-core/package.json dependencies');
    console.error('Or add to ALLOWLIST if they are optional dev-time tools (wrap in try-catch)\n');
    process.exit(1);
  }

  console.log('PASS: All script dependencies are declared in .yard-core/package.json\n');
  process.exit(0);
}
