...
const EXCLUDE_PATTERNS = [
  ...
  /docs\/standards\/YARD-LIVRO-DE-OURO/,
  /docs\/standards\/YARD-FRAMEWORK-MASTER\.md$/,
...
async function generateManifest() {
  ...
  if (!fs.existsSync(yardCoreDir)) {
    throw new Error(`.yard-core directory not found at ${yardCoreDir}`);
  }
  ...
  console.log(`Generating manifest for yard-core v${version}...`);
...
async function writeManifest(manifest) {
  const manifestPath = path.join(__dirname, '..', '.yard-core', 'install-manifest.yaml');
  ...
  // Add header comment
  const header = `# YARD-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('YARD-Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
