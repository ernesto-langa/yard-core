...
#
# Usage:
#   .\scripts\sign-manifest.ps1 -SecretKey C:\path\to\yard-core.key
#
# Prerequisites:
...
$ManifestPath = Join-Path $ProjectRoot ".yard-core\install-manifest.yaml"
$SignaturePath = "$ManifestPath.minisig"

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host "  YARD-Core Manifest Signing Tool (Windows)" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host ""
...
if (-not (Test-Path $ManifestPath)) {
    ...
    Write-Host "  node bin\yard.js manifest:generate"
    exit 1
}
...
Push-Location (Join-Path $ProjectRoot ".yard-core")
try {
...
Write-Host "  1. git add .yard-core\install-manifest.yaml.minisig"
